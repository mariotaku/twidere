...
public abstract class PLAAbsListView extends PLAAdapterView<ListAdapter> implements
	 ...
	 * Acts upon click
	 */
	private PLAAbsListView.PerformClick mPerformClick;
	/**
	...
	public PLAAbsListView(final Context context) {
		super(context);
		initAbsListView();

		setVerticalScrollBarEnabled(true);
		// final TypedArray a = context.obtainStyledAttributes(styleable.View);
		// initializeScrollbars(a);
		// a.recycle();
	}

	public PLAAbsListView(final Context context, final AttributeSet attrs) {
		this(context, attrs, android.R.attr.absListViewStyle);
	}

	public PLAAbsListView(final Context context, final AttributeSet attrs, final int defStyle) {
	...
	public LayoutParams generateLayoutParams(final AttributeSet attrs) {
		return new PLAAbsListView.LayoutParams(getContext(), attrs);
	}
			...
			case MotionEvent.ACTION_MOVE: {
				switch (mTouchMode) {
					case TOUCH_MODE_DOWN:
						...
						if (startScrollIfNeeded(y - mMotionY)) return true;
						break;
				}
				break;
			}
			...
			case MotionEvent.ACTION_DOWN: {
				...
				if (!mDataChanged) {
					if (mTouchMode != TOUCH_MODE_FLING && motionPosition >= 0 && getAdapter().isEnabled(motionPosition)) {
						...
						if (ev.getEdgeFlags() != 0 && motionPosition < 0) // If
																			// we
																			// couldn't
																			// find
																			// a
				...
				switch (mTouchMode) {
					...
					case TOUCH_MODE_SCROLL:
						...
						if (y != mLastY) {
							...
							final int incrementalDeltaY = mLastY != Integer.MIN_VALUE ? y - mLastY : deltaY;
							...
							// Check to see if we have bumped into the scroll
							// limit
							if (atEdge && getChildCount() > 0) {
								// Treat this like we're starting a new scroll
								// from the current
								// position. This will let the user start
								// scrolling back into
								// content immediately rather than needing to
								// scroll back to the
				...
				switch (mTouchMode) {
					...
					case TOUCH_MODE_DONE_WAITING:
						...
						if (child != null && !child.hasFocusable()) {
							...
							final PLAAbsListView.PerformClick performClick = mPerformClick;
					...
					case TOUCH_MODE_SCROLL:
						...
						if (childCount > 0) {
							...
							final int bottom = getFillChildBottom();
	...
	public void reclaimViews(final List<View> views) {
		...
		for (int i = 0; i < childCount; i++) {
			final View child = getChildAt(i);
			final PLAAbsListView.LayoutParams lp = (PLAAbsListView.LayoutParams) child.getLayoutParams();
	 ...
	 *            in the recycler.
	 * 
	 * @see android.widget.PLAAbsListView.RecycleBin
	...
	public boolean showContextMenuForChild(final View originalView) {
		...
		if (longPressPosition >= 0) {
			...
			if (onItemLongClickListener != null) {
				handled = onItemLongClickListener.onItemLongClick(PLAAbsListView.this, originalView, longPressPosition,
						longPressId);
			}
			if (!handled) {
	...
	protected boolean checkLayoutParams(final ViewGroup.LayoutParams p) {
		return p instanceof PLAAbsListView.LayoutParams;
	}
	...
	@Override
	protected void handleDataChangedPLA() {
		final int count = mItemCount;
		if (count > 0) {
			...
			if (mNeedSync) {
				...
				// it
				mNeedSync = false;
				if (mTranscriptMode == TRANSCRIPT_MODE_ALWAYS_SCROLL || mTranscriptMode == TRANSCRIPT_MODE_NORMAL
			...
			if (!isInTouchMode()) {
				...
				selectablePos = lookForSelectablePositionPLA(newPos, true);

				// Looking down didn't work -- try looking up
				selectablePos = lookForSelectablePositionPLA(newPos, false);
				if (selectablePos >= 0) return;
			} else {
	 ...
	 * @param mSyncPosition
	 */
	protected void onLayoutSync(final int syncPosition) {
	}
	 ...
	 * @param mSyncPosition
	 */
	protected void onLayoutSyncFinished(final int syncPosition) {
	}
	...
	protected void tryOffsetChildrenTopAndBottom(final int offset) {
		if (DEBUG) {
	...
	private void dispatchFinishTemporaryDetach(final View v) {
		if (v == null) return;
	...
	private boolean performLongPress(final View child, final int longPressPosition, final long longPressId) {
		...
		final OnItemLongClickListener onItemLongClickListener = getOnItemLongClickListener();
		if (onItemLongClickListener != null) {
			handled = onItemLongClickListener.onItemLongClick(PLAAbsListView.this, child, longPressPosition,
					longPressId);
		}
		if (!handled) {
			mContextMenuInfo = createContextMenuInfo(child, longPressPosition, longPressId);
			handled = super.showContextMenuForChild(PLAAbsListView.this);
		}
		if (handled) {
	...
	private boolean startScrollIfNeeded(final int deltaY) {
	...
	void keyPressed() {
		...
		if (selector != null && (isFocused() || touchModeDrawsInPressedState()) && selectorRect != null
			...
			final int mSelectedPosition = mMotionPosition;
	...
	void positionSelector(final View sel) {
		final Rect selectorRect = mSelectorRect;
	...
	boolean trackMotionScroll(int deltaY, int incrementalDeltaY) {
		...
		if (firstPosition == 0 && firstTop >= listPadding.top && deltaY >= 0) // Don't
																				// need
																				// to
																				// move
																				// views
		...
		if (firstPosition + childCount == mItemCount && lastBottom <= end && deltaY <= 0) // Don't
																							// need
																							// to
																							// move
																							// views
			...
			for (int i = 0; i < childCount; i++) {
				...
				if (child.getBottom() >= top) {
					...
					if (position >= headerViewsCount && position < footerViewsStart) {
						...
						if (ViewDebug.TRACE_RECYCLER) {
							ViewDebug.trace(child, ViewDebug.RecyclerTraceType.MOVE_TO_SCRAP_HEAP, firstPosition + i,
									-1);
						}
					}
				}
	...
	private class CheckForKeyLongPress extends WindowRunnnable implements Runnable {
		...
		public void run() {
			final int mSelectedPosition = mMotionPosition;
			final long mSelectedRowId = getItemIdAtPosition(mSelectedPosition);
	...
	private class FlingRunnable implements Runnable {
		...
		void start(int initialVelocity) {
			...
			if (DEBUG) {
				Log.d(TAG,
						String.format("String Fling: [%d, %d] to [%d]", initialY, initialVelocity,
								mScroller.getFinalY()));
			}
	...
	class RecycleBin {
		...
		void addScrapView(final View scrap) {
			final PLAAbsListView.LayoutParams lp = (PLAAbsListView.LayoutParams) scrap.getLayoutParams();
			if (lp == null) return;
		...
		View getScrapView(final int position) {
			...
			if (mViewTypeCount == 1) {
				...
				if (whichScrap >= 0 && whichScrap < mScrapViews.length) {
					...
					final int size = scrapViews.size();
					if (size > 0) return scrapViews.remove(size - 1);
				}
			}
			return null;
		}
		...
		void scrapActiveViews() {
			...
			for (int i = count - 1; i >= 0; i--) {
				...
				if (victim != null) {
					final int whichScrap = ((PLAAbsListView.LayoutParams) victim.getLayoutParams()).viewType;
