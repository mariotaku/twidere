...
public class DragSortListView extends ListView {
	...
	public DragSortListView(final Context context, final AttributeSet attrs) {
		...
		if (attrs != null) {
			...
			mItemHeightCollapsed = Math
					.max(1, a.getDimensionPixelSize(R.styleable.DragSortListView_collapsedHeight, 1));

			mTrackDragSort = a.getBoolean(R.styleable.DragSortListView_trackDragSort, false);
			...
			mSlideRegionFrac = Math.max(0.0f,
					Math.min(1.0f, 1.0f - a.getFloat(R.styleable.DragSortListView_slideShuffleSpeed, 0.75f)));

			mAnimate = mSlideRegionFrac > 0.0f;

			final float frac = a.getFloat(R.styleable.DragSortListView_dragScrollStart, mDragUpScrollStartFrac);

			setDragScrollStart(frac);

			mMaxScrollSpeed = a.getFloat(R.styleable.DragSortListView_maxDragScrollSpeed, mMaxScrollSpeed);

			removeAnimDuration = a.getInt(R.styleable.DragSortListView_removeAnimationDuration, removeAnimDuration);

			dropAnimDuration = a.getInt(R.styleable.DragSortListView_dropAnimationDuration, dropAnimDuration);

			final boolean useDefault = a.getBoolean(R.styleable.DragSortListView_useDefaultController, true);

			if (useDefault) {
				final boolean removeEnabled = a.getBoolean(R.styleable.DragSortListView_removeEnabled, false);
				final int removeMode = a.getInt(R.styleable.DragSortListView_removeMode,
						DragSortController.FLING_REMOVE);
				final boolean sortEnabled = a.getBoolean(R.styleable.DragSortListView_sortEnabled, true);
				final int dragInitMode = a.getInt(R.styleable.DragSortListView_dragStartMode,
						DragSortController.ON_DOWN);
				final int dragHandleId = a.getResourceId(R.styleable.DragSortListView_dragHandleId, 0);
				final int flingHandleId = a.getResourceId(R.styleable.DragSortListView_flingHandleId, 0);
				final int clickRemoveId = a.getResourceId(R.styleable.DragSortListView_clickRemoveId, 0);
				final int bgColor = a.getColor(R.styleable.DragSortListView_floatBackgroundColor, Color.BLACK);

				final DragSortController controller = new DragSortController(this, dragHandleId, dragInitMode,
						removeMode, clickRemoveId, flingHandleId);
	...
	public void moveCheckState(final int from, final int to) {
		...
		if (runCount == 1 && runStart[0] == runEnd[0]) // Special case where all
														// items are checked, we
														// can never set any
	 ...
	 * {@link DropListener#onDrop(int, int)}.
	 * 
	...
	public boolean onTouchEvent(final MotionEvent ev) {
		...
		if (!mDragEnabled) return super.onTouchEvent(ev);
	...
	public boolean startDrag(final int position, final int dragFlags, final int deltaX, final int deltaY) {
		if (!mInTouchEvent || mFloatViewManager == null) return false;
	 ...
	 *         in progress.
	 */
	public boolean startDrag(final int position, final View floatView, final int dragFlags, final int deltaX,
			final int deltaY) {
	...
	private int getChildHeight(final int position) {
		...
		if (v != null)
			...
			if (childHeight != -1) // Log.d("mobeta",
									// "found child height in cache!");
				return childHeight;
	...
	private int getItemHeight(final int position) {
		...
		if (v != null) // item is onscreen, just get the height of the View
			return v.getHeight();
		else
	...
	private int getShuffleEdge(final int position, final int top) {
		...
		if (position <= numHeaders || position >= getCount() - numFooters) return top;
	...
	private static int buildRunList(final SparseBooleanArray cip, final int rangeStart, final int rangeEnd,
			final int[] runStart, final int[] runEnd) {
		int runCount = 0;
	...
	private static int findFirstSetIndex(final SparseBooleanArray sba, final int rangeStart, final int rangeEnd) {
		...
		if (i == size || sba.keyAt(i) >= rangeEnd) return -1;
		return i;
	}
	...
	public interface FloatViewManager {
		 ...
		 * some for you (with a width of MATCH_PARENT and a height of
		 * WRAP_CONTENT).
		 * 
