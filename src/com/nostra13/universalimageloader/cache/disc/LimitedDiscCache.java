...
public abstract class LimitedDiscCache extends BaseDiscCache {
	 ...
	 */
	public LimitedDiscCache(final File cacheDir, final FileNameGenerator fileNameGenerator, final int sizeLimit) {
	 ...
	 *            with the most oldest last usage date will be deleted.
	 */
	public LimitedDiscCache(final File cacheDir, final int sizeLimit) {
	...
	public void put(final String key, final File file) {
		final int valueSize = getSize(file);
		int curCacheSize = cacheSize.get();

		while (curCacheSize + valueSize > sizeLimit) {
			final int freedSize = removeNext();
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				if (cachedFiles != null) { // rarely but it can happen, don't
											// know why
					for (final File cachedFile : cachedFiles) {
	...
	private int removeNext() {
		if (lastUsageDates.isEmpty()) return INVALID_SIZE;
		Long oldestUsage = null;
		File mostLongUsedFile = null;
		final Set<Entry<File, Long>> entries = lastUsageDates.entrySet();
		synchronized (lastUsageDates) {
			for (final Entry<File, Long> entry : entries) {
