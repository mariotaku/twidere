...
public class ImageLoader {
	 ...
	 */
	public void denyNetworkDownloads(final boolean denyNetworkDownloads) {
	 ...
	 * @throws IllegalArgumentException if passed <b>imageView</b> is null
	 */
	public void displayImage(final String uri, final ImageView imageView, DisplayImageOptions options,
			ImageLoadingListener listener) {
		checkConfiguration();
		if (imageView == null) throw new IllegalArgumentException(ERROR_WRONG_ARGUMENTS);
		if (listener == null) {
		...
		final ImageSize targetSize = ImageSizeUtils.defineTargetSizeForView(imageView,
				configuration.maxImageWidthForMemoryCache, configuration.maxImageHeightForMemoryCache);
		final String memoryCacheKey = MemoryCacheUtil.generateKey(uri, targetSize);
		engine.prepareDisplayTaskFor(imageView, memoryCacheKey);

		listener.onLoadingStarted(uri, imageView);
		final Bitmap bmp = configuration.memoryCache.get(memoryCacheKey);
		if (bmp != null && !bmp.isRecycled()) {
			...
			if (options.shouldPostProcess()) {
				final ImageLoadingInfo imageLoadingInfo = new ImageLoadingInfo(uri, imageView, targetSize,
						memoryCacheKey, options, listener, engine.getLockForUri(uri));
				final ProcessAndDisplayImageTask displayTask = new ProcessAndDisplayImageTask(engine, bmp,
			...
			final ImageLoadingInfo imageLoadingInfo = new ImageLoadingInfo(uri, imageView, targetSize, memoryCacheKey,
					options, listener, engine.getLockForUri(uri));
			final LoadAndDisplayImageTask displayTask = new LoadAndDisplayImageTask(engine, imageLoadingInfo,
					options.getHandler());
			engine.submit(displayTask);
		}
	}
	 ...
	 * @throws IllegalArgumentException if passed <b>imageView</b> is null
	 */
	public void displayImage(final String uri, final ImageView imageView, final ImageLoadingListener listener) {
	 ...
	 *            - otherwise.
	 */
	public void handleSlowNetwork(final boolean handleSlowNetwork) {
	 ...
	 *             null
	 */
	public synchronized void init(final ImageLoaderConfiguration configuration) {
		if (configuration == null) throw new IllegalArgumentException(ERROR_INIT_CONFIG_WITH_NULL);
	 ...
	 *             method wasn't called before
	 */
	public void loadImage(final String uri, final DisplayImageOptions options, final ImageLoadingListener listener) {
	 ...
	 *             method wasn't called before
	 */
	public void loadImage(final String uri, final ImageLoadingListener listener) {
	 ...
	 *             method wasn't called before
	 */
	public void loadImage(final String uri, ImageSize targetImageSize, DisplayImageOptions options,
		...
		if (options.getDisplayer() instanceof FakeBitmapDisplayer) {
			...
			optionsWithFakeDisplayer = new DisplayImageOptions.Builder().cloneFrom(options)
					.displayer(fakeBitmapDisplayer).build();
		}

		final ImageView fakeImage = new ImageView(configuration.context);
	 ...
	 * @param uri Image URI (i.e. "http://site.com/image.png",
	 *            "file:///mnt/sdcard/image.png")
	 * @param minImageSize Minimal size for {@link Bitmap} which will be
	 *            returned in
	 *            {@linkplain ImageLoadingListener#onLoadingComplete(String, android.view.View, android.graphics.Bitmap)}
	 *            callback}. Downloaded image will be decoded and scaled to
	 *            {@link Bitmap} of the size which is <b>equal or larger</b>
	 ...
	 *             method wasn't called before
	 */
	public void loadImage(final String uri, final ImageSize minImageSize, final ImageLoadingListener listener) {
	...
	private void checkConfiguration() {
		if (configuration == null) throw new IllegalStateException(ERROR_NOT_INIT);
	}
