...
final class LoadAndDisplayImageTask implements Runnable {
    ...
    public LoadAndDisplayImageTask(final ImageLoaderEngine engine,
            final ImageLoadingInfo imageLoadingInfo,
    ...
    public void run() {
        ...
        try {
            ...
            if (bmp == null) {
                ...
                if (imageViewCollected)
                    return; // listener callback already was
                            // fired
                if (bmp == null)
        ...
        final DisplayBitmapTask displayBitmapTask = new DisplayBitmapTask(bmp, imageLoadingInfo,
                engine, loadedFrom);
    ...
    private Bitmap decodeImage(final String imageUri) throws IOException {
        ...
        final ImageDecodingInfo decodingInfo = new ImageDecodingInfo(memoryCacheKey, imageUri,
                targetSize,
    ...
    private boolean downloadSizedImage(final File targetFile, final int maxWidth,
            final int maxHeight)
            throws IOException {
        // Download, decode, compress and save image
        final ImageSize targetImageSize = new ImageSize(maxWidth, maxHeight);
        final DisplayImageOptions specialOptions = new DisplayImageOptions.Builder()
                .cloneFrom(options)
                .imageScaleType(ImageScaleType.IN_SAMPLE_INT).build();
        final ImageDecodingInfo decodingInfo = new ImageDecodingInfo(memoryCacheKey, uri,
                targetImageSize,
    ...
    private Bitmap tryLoadBitmap() {
        ...
        try {
            ...
            if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
                ...
                final String imageUriForDecoding = options.isCacheOnDisc() ? tryCacheImageOnDisc(imageFile)
                        : uri;
                if (!checkTaskIsNotActual()) {
    ...
    private static class StreamCopyListenerImpl implements IoUtils.StreamCopyListener {
        ...
        public void onUpdate(final int current, final int total) {
            if (task.checkTaskIsImageViewReused() || task.checkTaskIsInterrupted())
                return;
        ...
        private static class UpdateRunnable implements Runnable {
            ...
            UpdateRunnable(final ImageLoadingListener listener, final String uri,
