...
public class BaseImageDecoder implements ImageDecoder {
    ...
    public Bitmap decode(final ImageDecodingInfo decodingInfo) throws IOException {
        ...
        final ImageFileInfo imageInfo = defineImageSizeAndRotation(imageStream,
        ...
        if (decodedBitmap == null) {
            ...
            decodedBitmap = considerExactScaleAndOrientaiton(decodedBitmap, decodingInfo,
                    imageInfo.exif.rotation,
                    imageInfo.exif.flipHorizontal);
        }
        return decodedBitmap;
    }

    protected Bitmap considerExactScaleAndOrientaiton(final Bitmap subsampledBitmap,
            final ImageDecodingInfo decodingInfo, final int rotation, final boolean flipHorizontal) {
        final Matrix m = new Matrix();
        // Scale to exact size if need
        final ImageScaleType scaleType = decodingInfo.getImageScaleType();
        if (scaleType == ImageScaleType.EXACTLY || scaleType == ImageScaleType.EXACTLY_STRETCHED) {
            final ImageSize srcSize = new ImageSize(subsampledBitmap.getWidth(),
                    subsampledBitmap.getHeight(), rotation);
            final float scale = ImageSizeUtils.computeImageScale(srcSize,
        ...
        final Bitmap finalBitmap = Bitmap.createBitmap(subsampledBitmap, 0, 0,
    ...
    protected Bitmap decodeStream(final InputStream imageStream, final Options decodingOptions)
            throws IOException {
        try {
    ...
    protected ImageFileInfo defineImageSizeAndRotation(final InputStream imageStream,
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ECLAIR) {
            exif = ExifAccessorEclair.defineExifOrientation(imageUri, options.outMimeType);
        } else {
            exif = new ExifInfo();
        }
    ...
    protected Options prepareDecodingOptions(final ImageSize imageSize,
            final ImageDecodingInfo decodingInfo) {
        final ImageScaleType scaleType = decodingInfo.getImageScaleType();
    ...
    private static final class ExifAccessorEclair {
        protected static ExifInfo defineExifOrientation(final String imageUri, final String mimeType) {
            int rotation = 0;
            boolean flip = false;
            if ("image/jpeg".equalsIgnoreCase(mimeType) && Scheme.ofUri(imageUri) == Scheme.FILE) {
                try {
                    ...
                    final int exifOrientation = exif.getAttributeInt(ExifInterface.TAG_ORIENTATION,
