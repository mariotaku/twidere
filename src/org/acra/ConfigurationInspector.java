...
public class ConfigurationInspector {
	 ...
	 * values.
	 * 
	 * @param valueNames The array containing the different values and names for
	 *            this field. Must contain mask values too.
	...
	private static String getFieldValueName(Configuration conf, Field f) throws IllegalArgumentException,
		...
		if (fieldName.equals(FIELD_MCC) || fieldName.equals(FIELD_MNC))
			...
			if (values == null) // Unknown field, return the raw int as String
				return Integer.toString(f.getInt(conf));
			final String value = values.get(f.getInt(conf));
			if (value == null) // Unknown value, return the raw int as String
				return Integer.toString(f.getInt(conf));
			return value;
		}
	}
