...
public class CrashReportData extends EnumMap<ReportField, String> {
	 ...
	 * 
	 * @param out the {@code PrintStream} to write the content to in human
	 *            readable form.
	 */
	public void list(PrintStream out) {
		if (out == null) throw new NullPointerException();
		final StringBuilder buffer = new StringBuilder(80);
		final Enumeration<ReportField> keys = keys();
		while (keys.hasMoreElements()) {
			final ReportField key = keys.nextElement();
			buffer.append(key);
			buffer.append('=');
			String property = super.get(key);
	 ...
	 * 
	 * @param writer the {@code PrintWriter} to write the content to in human
	 *            readable form.
	 */
	 ...
	 * @throws IOException if error occurs during reading from the
	 *             {@code InputStream}.
	 */
	...
	public synchronized void load(Reader reader) throws IOException {
		...
		while (true) {
			...
			if (mode == UNICODE) {
				...
				} else if (count <= 4) // luni.09=Invalid Unicode sequence:
										// illegal character
				...
				switch (nextChar) {
					...
					case '\r':
						...
						if (offset > 0 || offset == 0 && keyLength == 0) {
							...
							put(Enum.valueOf(ReportField.class, temp.substring(0, keyLength)),
		...
		if (mode == UNICODE && count <= 4) // luni.08=Invalid Unicode sequence:
	...
	public synchronized void store(OutputStream out, String comment) throws IOException {
		...
		for (final Map.Entry<ReportField, String> entry : entrySet()) {
	...
	public synchronized void storeToXML(OutputStream os, String comment, String encoding) throws IOException {
		...
		if (os == null || encoding == null) throw new NullPointerException();
		...
		final PrintStream printStream = new PrintStream(os, false, encodingCanonicalName);
	...
	private boolean isEbcdic(BufferedInputStream in) throws IOException {
		...
		while ((b = (byte) in.read()) != -1) {
			if (b == 0x23 || b == 0x0a || b == 0x3d) return false;
			if (b == 0x15) return true;
		}
