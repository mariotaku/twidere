...
public class FTP extends SocketClient {
	...
	public int allo(final int bytes, final int recordSize) throws IOException {
		return sendCommand(FTPCommand.ALLO, Integer.toString(bytes) + " R " + Integer.toString(recordSize));
	}
	 ...
	 * <p>
	 * 
	 * @param pathname The pathname to use for the file when stored at the
	 ...
	 *                code 421. This exception may be caught either as an
	 *                IOException or independently as itself.
	 * @exception IOException If an I/O error occurs while receiving the server
	 *                reply.
	 ***/
	...
	public String getReplyString() {
		...
		if (!_newReplyString) return _replyString;
	 ...
	 * <p>
	 * 
	 * @param pathname The pathname to list, may be {@code null} in which case
	 *            the command is sent with no parameters
	 * @return The reply code received from the server.
	 * @exception FTPConnectionClosedException If the FTP server prematurely
	 *                closes the connection as a result of the client being idle
	 *                or some other reason causing the server to send FTP reply
	...
	public int mode(final int mode) throws IOException {
		return sendCommand(FTPCommand.MODE, __modes.substring(mode, mode + 1));
	}
	 ...
	 * the reply, and return the reply code.
	 * 
	 * @param password The plain text password of the username being logged
	 *            into.
	 * @return The reply code received from the server.
	 * @exception FTPConnectionClosedException If the FTP server prematurely
	 *                closes the connection as a result of the client being idle
	 ...
	 * @param command The FTPCommand constant corresponding to the FTP command
	 *            to send.
	 * @param args The arguments to the FTP command. If this parameter is set to
	...
	public int sendCommand(final String command, final String args) throws IOException {
		if (_controlOutput_ == null) throw new IOException("Connection is not open");
	 ...
	 * level details for you.
	 * 
	 * @param pathname The base pathname to use for the file when stored at the
	...
	public int stru(final int structure) throws IOException {
		return sendCommand(FTPCommand.STRU, __modes.substring(structure, structure + 1));
	}
	...
	protected void __noop() throws IOException {
		final String msg = __buildMessage(FTPCommand.getCommand(FTPCommand.NOOP), null);
		__send(msg);
	...
	private void __getReply(final boolean reportReply) throws IOException {
		...
		if (line == null) throw new FTPConnectionClosedException("Connection closed without indication.");
