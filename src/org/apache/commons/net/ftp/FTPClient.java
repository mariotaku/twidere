...
public class FTPClient extends FTP implements Configurable {
	...
	private boolean __useEPSVwithIPv4; // whether to attempt EPSV with an IPv4
										// connection
	 ...
	 * @exception FTPConnectionClosedException If the FTP server prematurely
	 *                closes the connection as a result of the client being idle
	 *                or some other reason causing the server to send FTP reply
	 *                code 421. This exception may be caught either as an
	 ...
	 *                code 421. This exception may be caught either as an
	 *                IOException or independently as itself.
	 * @exception org.apache.commons.net.io.CopyStreamException If an I/O error
	 *                occurs while actually transferring the file. The
	 *                CopyStreamException allows you to determine the number of
	 *                bytes transferred and the IOException causing the error.
	 *                This exception may be caught either as an IOException or
	...
	public boolean enterRemotePassiveMode() throws IOException {
		if (pasv() != FTPReply.ENTERING_PASSIVE_MODE) return false;
	...
	public String[] featureValues(final String feature) throws IOException {
		if (!initFeatureMap()) return null;
		final Set<String> entries = __featuresMap.get(feature.toUpperCase(Locale.ENGLISH));
		if (entries != null) return entries.toArray(new String[entries.size()]);
		return null;
	}
	...
	public String getModificationTime(final String pathname) throws IOException {
		if (FTPReply.isPositiveCompletion(mdtm(pathname))) return getReplyString();
		return null;
	}
	...
	public String getStatus() throws IOException {
		if (FTPReply.isPositiveCompletion(stat())) return getReplyString();
		return null;
	}
	...
	public String getStatus(final String pathname) throws IOException {
		if (FTPReply.isPositiveCompletion(stat(pathname))) return getReplyString();
		return null;
	}
	 ...
	 *                (and the default system type property is not defined)
	 * @since 2.2
	 ***/
	...
	public boolean hasFeature(final String feature) throws IOException {
		if (!initFeatureMap()) return false;
	...
	public boolean hasFeature(final String feature, final String value) throws IOException {
		if (!initFeatureMap()) return false;
		final Set<String> entries = __featuresMap.get(feature.toUpperCase(Locale.ENGLISH));
		if (entries != null) return entries.contains(value);
		return false;
	}
	 ...
	 *            <code> FTPFileEntryParser </code> that should be used to parse
	 *            each server file listing. May be {@code null}, in which case
	 *            the code checks first the system property
	 ...
	 * <p>
	 * 
	 * @param pathname The file or directory to list. Since the server may or
	 *            may not expand glob expressions, using them here is not
	...
	public String listHelp() throws IOException {
		if (FTPReply.isPositiveCompletion(help())) return getReplyString();
		return null;
	}
	...
	public String listHelp(final String command) throws IOException {
		if (FTPReply.isPositiveCompletion(help(command))) return getReplyString();
		return null;
	}
	...
	public String[] listNames(final String pathname) throws IOException {
		...
		if ((socket = _openDataConnection_(FTPCommand.NLST, getListArguments(pathname))) == null) return null;
	...
	public boolean login(final String username, final String password) throws IOException {
		...
		if (FTPReply.isPositiveCompletion(_replyCode)) return true;

		// If we get here, we either have an error code, or an intermmediate
		// reply requesting password.
		if (!FTPReply.isPositiveIntermediate(_replyCode)) return false;
	...
	public FTPFile mlistFile(final String pathname) throws IOException {
		final boolean success = FTPReply.isPositiveCompletion(sendCommand(FTPCommand.MLST, pathname));
		if (success) {
			final String entry = getReplyStrings()[1].substring(1); // skip
																	// leading
																	// space for
																	// parser
	...
	public String printWorkingDirectory() throws IOException {
		if (pwd() != FTPReply.PATHNAME_CREATED) return null;
	 ...
	 * <p>
	 * 
	 * @param filename The name of the file to be appended to, or if the file
	 *            does not exist, the name to call the file being stored.
	 *            <p>
	 * @return True if successfully completed, false if not.
	 * @exception FTPConnectionClosedException If the FTP server prematurely
	 *                closes the connection as a result of the client being idle
	 ...
	 * <p>
	 * 
	 * @param filename The name on which to base the filename of the file that
	 *            is to be stored.
	 * @return True if successfully completed, false if not.
	 * @exception FTPConnectionClosedException If the FTP server prematurely
	 *                closes the connection as a result of the client being idle
	...
	public boolean rename(final String from, final String to) throws IOException {
		if (!FTPReply.isPositiveIntermediate(rnfr(from))) return false;
	 ...
	 * supported).
	 * 
	 * @param autodetect If true, automatic server encoding detection will be
	 *            enabled.
	 */
	 ...
	 * ACTIVE_LOCAL_DATA_CONNECTION_MODE.
	 * 
	 * @param fileType The <code> _FILE_TYPE </code> constant indcating the type
	 *            of file.
	 * @return True if successfully completed, false if not.
	 * @exception FTPConnectionClosedException If the FTP server prematurely
	 *                closes the connection as a result of the client being idle
	 ...
	 * <p>
	 * 
	 * @param offset The offset into the remote file at which to start the next
	...
	protected void _connectAction_() throws IOException {
		...
		if (__autodetectEncoding) {
			final ArrayList<String> oldReplyLines = new ArrayList<String>(_replyLines);
	...
	protected Socket _openDataConnection_(final String command, final String arg) throws IOException {
		if (__dataConnectionMode != ACTIVE_LOCAL_DATA_CONNECTION_MODE
				&& __dataConnectionMode != PASSIVE_LOCAL_DATA_CONNECTION_MODE) return null;
		...
		if (__dataConnectionMode == ACTIVE_LOCAL_DATA_CONNECTION_MODE) {
			...
			try {
				...
				if (__restartOffset > 0 && !restart(__restartOffset)) return null;

				if (!FTPReply.isPositivePreliminary(sendCommand(command, arg))) return null;
			...
			if (attemptEPSV && epsv() == FTPReply.ENTERING_EPSV_MODE) {
				...
				if (isInet6Address) return null; // Must use EPSV for IPV6
				// If EPSV failed on IPV4, revert to PASV
				if (pasv() != FTPReply.ENTERING_PASSIVE_MODE) return null;
				_parsePassiveModeReply(_replyLines.get(0));
			}
	...
	protected void _parsePassiveModeReply(final String reply) throws MalformedServerReplyException {
		...
		try {
			final InetAddress host = InetAddress.getByName(__passiveHost);
			...
			if (host.isSiteLocalAddress() && !getRemoteAddress().isSiteLocalAddress()) {
				final String hostAddress = getRemoteAddress().getHostAddress();
				fireReplyReceived(0, "[Replacing site local address " + __passiveHost + " with " + hostAddress + "]\n");
				__passiveHost = hostAddress;
			}
		} catch (final UnknownHostException e) { // Should not happen as we are
	...
	protected boolean _retrieveFile(final String command, final String remote, final OutputStream local)
		...
		if ((socket = _openDataConnection_(command, remote)) == null) return false;
		...
		final boolean ok = completePendingCommand();
		return ok;
	}
	...
	protected InputStream _retrieveFileStream(final String command, final String remote) throws IOException {
		...
		if ((socket = _openDataConnection_(command, remote)) == null) return null;
	...
	private CopyStreamListener __mergeListeners(final CopyStreamListener local) {
		if (local == null) return __copyStreamListener;
		if (__copyStreamListener == null) return local;
		// Both are non-null
	...
	private String __parsePathname(final String reply) {
		final int begin = reply.indexOf('"') + 1;
		final int end = reply.indexOf('"', begin);
	...
	private int getActivePort() {
		if (__activeMinPort > 0 && __activeMaxPort >= __activeMinPort) {
			if (__activeMaxPort == __activeMinPort) return __activeMaxPort;
	...
	private boolean initFeatureMap() throws IOException {
		if (__featuresMap == null) {
			...
			final boolean success = FTPReply.isPositiveCompletion(feat());
	...
	private FTPListParseEngine initiateListParsing(final FTPFileEntryParser parser, final String pathname)
		...
		if ((socket = _openDataConnection_(FTPCommand.LIST, getListArguments(pathname))) == null) return engine;
	...
	private FTPListParseEngine initiateMListParsing(final String pathname) throws IOException {
		Socket socket;
		final FTPListParseEngine engine = new FTPListParseEngine(MLSxEntryParser.getInstance());
		if ((socket = _openDataConnection_(FTPCommand.MLSD, pathname)) == null) return engine;
	...
	private static class CSL implements CopyStreamListener {
		...
		CSL(final FTPClient parent, final long idleTime, final int maxWait) throws SocketException {
			idle = idleTime;
			this.parent = parent;
			currentSoTimeout = parent.getSoTimeout();
