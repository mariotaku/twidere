...
public class MLSxEntryParser extends FTPFileEntryParserImpl {
	...
	private static int UNIX_PERMS[][] = { // perm bits, broken down by octal int
											// value
	/* 0 */{},
	...
	public FTPFile parseFTPEntry(final String entry) {
		...
		for (final String fact : facts) {
			...
			if ("size".equals(factname)) {
				...
				final TimeZone GMT = TimeZone.getTimeZone("GMT"); // both need
																	// to be set
																	// for the
																	// parse to
																	// work OK
				...
				if ("group".equals(unixfact)) {
					...
					final int off = factvalue.length() - 3; // only parse last 3
															// digits
					for (int i = 0; i < 3; i++) {
