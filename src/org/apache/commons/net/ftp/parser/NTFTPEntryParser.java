...
public class NTFTPEntryParser extends ConfigurableFTPFileEntryParserImpl {
	...
	private static final String REGEX = "(\\S+)\\s+(\\S+)\\s+" // MM-dd-yy
																...
																// trailing
																// spaces
			+ "(?:(<DIR>)|([0-9]+))\\s+" // <DIR> or ddddd; swallow trailing
											// spaces
	...
	public NTFTPEntryParser(final FTPClientConfig config) {
		super(REGEX);
		configure(config);
		final FTPClientConfig config2 = new FTPClientConfig(FTPClientConfig.SYST_NT, DEFAULT_DATE_FORMAT2, null, null,
				null, null);
		config2.setDefaultDateFormatStr(DEFAULT_DATE_FORMAT2);
		timestampParser = new FTPTimestampParserImpl();
		((Configurable) timestampParser).configure(config2);
	}
	...
	public FTPFile parseFTPEntry(final String entry) {
		...
		if (matches(entry)) {
			...
			if (null == name || name.equals(".") || name.equals("..")) return null;
			f.setName(name);
