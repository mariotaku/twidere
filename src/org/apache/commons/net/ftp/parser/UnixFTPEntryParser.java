...
public class UnixFTPEntryParser extends ConfigurableFTPFileEntryParserImpl {
	...
	private static final String REGEX = "([bcdelfmpSs-])"
			...
			+ "(\\d+(?:,\\s*\\d+)?)\\s+" // size or n,m
			/*
			 * numeric or standard format date: yyyy-mm-dd (expecting hh:mm to
			 * follow) MMM [d]d [d]d MMM N.B. use non-space for MMM to allow for
			 * languages such as German which use diacritics (e.g. umlaut) in
			 * some abbreviations.
			 */
			+ "((?:\\d+[-/]\\d+[-/]\\d+)|(?:\\S{3}\\s+\\d{1,2})|(?:\\d{1,2}\\s+\\S{3}))\\s+"
			/*
			 * year (for non-recent standard format) - yyyy or time (for numeric
			 * or recent standard format) [h]h:mm
			 */
			+ "(\\d+(?::\\d+)?)\\s+"
	 ...
	 * 
	 * @exception IllegalArgumentException Thrown if the regular expression is
	 *                unparseable. Should not be seen under normal conditions.
	 *                It it is seen, this is a sign that <code>REGEX</code> is
	...
	public FTPFile parseFTPEntry(final String entry) {
		...
		if (matches(entry)) {
			...
			final String endtoken = group(22);
