...
public class OverScroller {
	 ...
	 * @param interpolator The scroll interpolator. If null, a default (viscous)
	 ...
	 * @param interpolator The scroll interpolator. If null, a default (viscous)
	 *            interpolator will be used.
	 * @param flywheel If true, successive fling motions will keep on increasing
	 *            scroll speed.
	 * @hide
	 */
	public OverScroller(final Context context, final Interpolator interpolator, final boolean flywheel) {
	 ...
	 * @param interpolator The scroll interpolator. If null, a default (viscous)
	 *            interpolator will be used.
	 * @param bounceCoefficientX A value between 0 and 1 that will determine the
	 *            proportion of the velocity which is preserved in the bounce
	 *            when the horizontal edge is reached. A null value means no
	 *            bounce. This behavior is no longer supported and this
	 *            coefficient has no effect.
	 * @param bounceCoefficientY Same as bounceCoefficientX but for the vertical
	 *            direction. This behavior is no longer supported and this
	 *            coefficient has no effect. !deprecated Use {!link
	 *            #OverScroller(Context, Interpolator, boolean)} instead.
	 */
	public OverScroller(final Context context, final Interpolator interpolator, final float bounceCoefficientX,
			final float bounceCoefficientY) {
		this(context, interpolator, true);
	}
	 ...
	 */
	public OverScroller(final Context context, final Interpolator interpolator, final float bounceCoefficientX,
			final float bounceCoefficientY, final boolean flywheel) {
	 ...
	 *            direction will be possible.
	 */
	public void fling(final int startX, final int startY, int velocityX, int velocityY, final int minX, final int maxX,
			final int minY, final int maxY, final int overX, final int overY) {
		// Continue a scroll or fling in progress
		if (mFlywheel && !isFinished()) {
			final float oldVelocityX = mScrollerX.mCurrVelocity;
			final float oldVelocityY = mScrollerY.mCurrVelocity;
	...
	public boolean isOverScrolled() {
		return !mScrollerX.mFinished && mScrollerX.mState != SplineOverScroller.SPLINE || !mScrollerY.mFinished
	 ...
	 */
	public boolean springBack(final int startX, final int startY, final int minX, final int maxX, final int minY,
			final int maxY) {
		mMode = FLING_MODE;
	...
	void setInterpolator(final Interpolator interpolator) {
		mInterpolator = interpolator;
	}
	...
	static class SplineOverScroller {
		...
		private void fitOnBounceCurve(final int start, final int end, final int velocity) {
			...
			final float totalDuration = (float) Math.sqrt(2.0 * (distanceToApex + distanceToEdge)
		...
		private void startAfterEdge(final int start, final int min, final int max, final int velocity) {
			...
			final boolean keepIncreasing = overDistance * velocity >= 0;
			if (keepIncreasing) {
		...
		void extendDuration(final int extend) {
			final long time = AnimationUtils.currentAnimationTimeMillis();
		...
		void fling(final int start, final int velocity, final int min, final int max, final int over) {
			mOver = over;
			mFinished = false;
		...
		void notifyEdgeReached(final int start, final int end, final int over) {
		...
		boolean springback(final int start, final int min, final int max) {
			mFinished = true;
		...
		void startScroll(final int start, final int distance, final int duration) {
			mFinished = false;
		 ...
		 */
		static private float getDeceleration(final int velocity) {
