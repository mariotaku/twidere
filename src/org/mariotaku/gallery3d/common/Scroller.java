...
public class Scroller {
	...
	public void fling(final int startX, final int startY, int velocityX, int velocityY, final int minX, final int maxX,
		...
		if (mFlywheel && !mFinished) {
			final float oldVel = getCurrVelocity();

			final float dx = mFinalX - mStartX;
			final float dy = mFinalY - mStartY;
			final float hyp = FloatMath.sqrt(dx * dx + dy * dy);

			final float ndx = dx / hyp;
			final float ndy = dy / hyp;

			final float oldVelocityX = ndx * oldVel;
			final float oldVelocityY = ndy * oldVel;
		...
		final float velocity = FloatMath.sqrt(velocityX * velocityX + velocityY * velocityY);
		...
		final float coeffX = velocity == 0 ? 1.0f : velocityX / velocity;
		final float coeffY = velocity == 0 ? 1.0f : velocityY / velocity;

		final int totalDistance = (int) (ALPHA * Math.exp(DECELERATION_RATE / (DECELERATION_RATE - 1.0) * l));
