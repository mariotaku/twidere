...
public class GLCanvasImpl implements GLCanvas {
    ...
    private static final float[] BOX_COORDINATES = {
            ...
            0, 0, 0, 1, 1, 1, 1, 0
    }; // used for drawing the outline of a
       // rectangle
    ...
    @Override
    public void drawTexture(final BasicTexture texture, final int x, final int y, final int width,
            final int height) {
    ...
    public void dumpStatisticsAndClear() {
        final String line = String.format(
                "MESH:%d, TEX_OES:%d, TEX_RECT:%d, FILL_RECT:%d, LINE:%d", mCountDrawMesh,
    ...
    private void drawBoundTexture(final BasicTexture texture, int x, int y, int width, int height) {
        ...
        if (isMatrixRotatedOrFlipped(mMatrixValues)) {
            if (texture.hasBorder()) {
                setTextureCoords(1.0f / texture.getTextureWidth(),
    ...
    private void drawTexture(final BasicTexture texture, final int x, final int y, final int width,
            final int height,
            final float alpha) {
        if (width <= 0 || height <= 0)
            return;
    ...
    private float[] mapPoints(final float m[], final int x1, final int y1, final int x2,
            final int y2) {
    ...
    private void setTextureCoords(final float left, final float top, final float right,
            final float bottom) {
    ...
    private static void convertCoordinate(final RectF source, final RectF target,
            final BasicTexture texture) {
    ...
    private static boolean isMatrixRotatedOrFlipped(final float matrix[]) {
        ...
        return Math.abs(matrix[MSKEW_X]) > eps || Math.abs(matrix[MSKEW_Y]) > eps
                || matrix[MSCALE_X] < -eps
                || matrix[MSCALE_Y] > eps;
    }
