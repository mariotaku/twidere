...
class PositionController {
    ...
    public boolean flingPage(int velocityX, int velocityY) {
        ...
        if (viewWiderThanScaledImage(mBox.mCurrentScale)
                && viewTallerThanScaledImage(mBox.mCurrentScale))
            return false;

        // We only allow flinging in the directions where it won't go over the
        // picture.
        final int edges = getImageAtEdges();
        if (velocityX > 0 && (edges & IMAGE_AT_LEFT_EDGE) != 0 || velocityX < 0
                && (edges & IMAGE_AT_RIGHT_EDGE) != 0) {
            velocityX = 0;
        }
        if (velocityY > 0 && (edges & IMAGE_AT_TOP_EDGE) != 0 || velocityY < 0
                && (edges & IMAGE_AT_BOTTOM_EDGE) != 0) {
            velocityY = 0;
        }
        ...
        mPageScroller.fling(p.mCurrentX, mBox.mCurrentY, velocityX, velocityY, mBoundLeft,
                mBoundRight, mBoundTop,
                mBoundBottom);
    ...
    private boolean setBoxSize(final int width, final int height, final boolean isViewSize) {
        ...
        if (width == mBox.mImageW && height == mBox.mImageH)
            return false;
        ...
        final float ratio = width > height ? (float) mBox.mImageW / width : (float) mBox.mImageH
                / height;
    ...
    private boolean startAnimation(final int targetX, final int targetY, final float targetScale,
            final int kind) {
        boolean changed = false;
    ...
    private boolean startOpeningAnimationIfNeeded() {
        ...
        mBox.mCurrentScale = Math.max(r.width() / (float) mBox.mImageW, r.height()
                / (float) mBox.mImageH);
    ...
    private class Platform extends Animatable {
        ...
        public boolean startSnapback() {
            ...
            final float scaleMin = mExtraScalingRange ? mBox.mScaleMin * SCALE_MIN_EXTRA
                    : mBox.mScaleMin;
            final float scaleMax = mExtraScalingRange ? mBox.mScaleMax * SCALE_MAX_EXTRA
                    : mBox.mScaleMax;
