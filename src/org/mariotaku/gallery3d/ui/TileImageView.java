...
public class TileImageView extends GLView {
    ...
    public TileImageView(final ImageViewerGLActivity context) {
        ...
        if (TILE_SIZE == 0) {
            ...
            sTilePool = ApiHelper.HAS_REUSING_BITMAP_IN_BITMAP_REGION_DECODER ? new BitmapPool(
                    BITMAP_SIZE,
    ...
    public boolean setPosition(final int centerX, final int centerY, final float scale,
            final int rotation) {
    ...
    @Override
    protected void onLayout(final boolean changeSize, final int left, final int top,
    ...
    private void drawTile(final GLCanvas canvas, final int tx, final int ty, final int level,
            final float x,
    ...
    private void getRange(final Rect out, final int cX, final int cY, final int level,
            final float scale,
            final int rotation) {
        ...
        final int width = (int) Math.ceil(Math.max(Math.abs(cos * w - sin * h),
                Math.abs(cos * w + sin * h)));
        final int height = (int) Math.ceil(Math.max(Math.abs(sin * w + cos * h),
    ...
    private void layoutTiles(final int centerX, final int centerY, final float scale,
            final int rotation) {
    ...
    private static boolean drawTile(Tile tile, final GLCanvas canvas, final RectF source,
            final RectF target) {
        while (true) {
            if (tile.isContentValid()) {
    ...
    private class Tile extends UploadedTexture {
        ...
        public String toString() {
            return String.format("tile(%s, %s, %s / %s)", mX / TILE_SIZE, mY / TILE_SIZE, mLevel,
                    mLevelCount);
        }
        ...
        boolean decode() {
            ...
            try {
                mDecodedTile = DecodeUtils.ensureGLCompatibleBitmap(mModel.getTile(mTileLevel, mX,
                        mY, TILE_SIZE,
