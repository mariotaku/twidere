...
abstract class UploadedTexture extends BasicTexture {
    ...
    private void uploadToCanvas(final GLCanvas canvas) {
        ...
        if (bitmap != null) {
            try {
                ...
                gl.glTexParameterfv(GL11.GL_TEXTURE_2D, GL11Ext.GL_TEXTURE_CROP_RECT_OES,
                        sCropRect, 0);
                ...
                if (bWidth == texWidth && bHeight == texHeight) {
                    ...
                    gl.glTexImage2D(GL11.GL_TEXTURE_2D, 0, format, texWidth, texHeight, 0, format,
                            type, null);
                    GLUtils.texSubImage2D(GL11.GL_TEXTURE_2D, 0, mBorder, mBorder, bitmap, format,
                            type);
                    ...
                    if (mBorder + bWidth < texWidth) {
                        ...
                        GLUtils.texSubImage2D(GL11.GL_TEXTURE_2D, 0, mBorder + bWidth, 0, line,
                                format, type);
                    }
                    ...
                    if (mBorder + bHeight < texHeight) {
                        ...
                        GLUtils.texSubImage2D(GL11.GL_TEXTURE_2D, 0, 0, mBorder + bHeight, line,
                                format, type);
                    }
                }
            } finally {
    ...
    private static Bitmap getBorderLine(final boolean vertical, final Config config,
            final int length) {
        ...
        if (bitmap == null) {
            bitmap = vertical ? Bitmap.createBitmap(1, length, config) : Bitmap.createBitmap(
                    length, 1, config);
