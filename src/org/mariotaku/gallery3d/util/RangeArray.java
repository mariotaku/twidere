...
public class RangeArray<T> {
	...
	public RangeArray(final int min, final int max) {
		mData = (T[]) new Object[max - min + 1];
		mOffset = min;
	}

	// Wraps around an existing array
	public RangeArray(final T[] src, final int min, final int max) {
		if (max - min + 1 != src.length) throw new AssertionError();
		mData = src;
		mOffset = min;
	}
	...
	public void put(final int i, final T object) {
