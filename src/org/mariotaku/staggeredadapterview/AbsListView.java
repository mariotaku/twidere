...
public abstract class AbsListView extends AdapterView<ListAdapter> implements TextWatcher,
	...
	public AbsListView(final Context context) {
		...
		// FIXME
		final TypedArray a = context.obtainStyledAttributes(new int[0]);
	...
	public AbsListView(final Context context, final AttributeSet attrs, final int defStyle) {
		...
		final boolean useTextFilter = a.getBoolean(styleable.AbsListView_textFilterEnabled, false);
		...
		final boolean enableFastScroll = a.getBoolean(styleable.AbsListView_fastScrollEnabled, false);
		setFastScrollEnabled(enableFastScroll);

		final boolean smoothScrollbar = a.getBoolean(styleable.AbsListView_smoothScrollbar, true);
		setSmoothScrollbarEnabled(smoothScrollbar);

		setChoiceMode(a.getInt(styleable.AbsListView_choiceMode, CHOICE_MODE_NONE));
		setFastScrollAlwaysVisible(a.getBoolean(styleable.AbsListView_fastScrollAlwaysVisible, false));
	...
	public CharSequence getTextFilter() {
		if (mTextFilterEnabled && mTextFilter != null) return mTextFilter.getText();
		return null;
	}
	...
	public int getVerticalScrollbarWidth() {
		if (isFastScrollAlwaysVisible()) return Math.max(super.getVerticalScrollbarWidth(), mFastScroller.getWidth());
				...
				case MotionEvent.ACTION_SCROLL: {
					if (mTouchMode == TOUCH_MODE_REST) {
						...
						if (vscroll != 0) {
							...
							if (!trackMotionScroll(delta, delta)) return true;
						}
					}
				}
			}
	...
	public boolean onInterceptTouchEvent(final MotionEvent ev) {
		...
		if (mFastScroller != null) {
			final boolean intercepted = mFastScroller.onInterceptTouchEvent(ev);
		...
		switch (action & MotionEvent.ACTION_MASK) {
			case MotionEvent.ACTION_DOWN: {
				...
				if (touchMode == TOUCH_MODE_FLING) return true;
				break;
			}
	...
	public Parcelable onSaveInstanceState() {
		...
		final boolean haveChildren = getChildCount() > 0 && mItemCount > 0;
		final long selectedId = getSelectedItemId();
		...
		if (mFiltered) {
			...
			if (textFilter != null) {
				final Editable filterText = textFilter.getText();
				if (filterText != null) {
	...
	public void onTextChanged(final CharSequence s, final int start, final int before, final int count) {
		if (mPopup != null && isTextFilterEnabled()) {
			...
			final boolean showing = mPopup.isShowing();
			if (!showing && length > 0) {
			...
			if (mAdapter instanceof Filterable) {
				final Filter f = ((Filterable) mAdapter).getFilter();
	...
	public boolean onTouchEvent(final MotionEvent ev) {
		if (!isEnabled()) // A disabled view that is clickable still consumes
							// the touch
			// events, it just doesn't respond to them.
			return isClickable() || isLongClickable();

		if (mFastScroller != null) {
			final boolean intercepted = mFastScroller.onTouchEvent(ev);
			if (intercepted) return true;
		}
					...
					default: {
						...
						if (!mDataChanged) {
							if (mTouchMode != TOUCH_MODE_FLING && motionPosition >= 0
								...
								// stopping a fling).
								// It might be a click or a scroll. Assume it is
								// a click until
								// proven otherwise
								mTouchMode = TOUCH_MODE_DOWN;
								// FIXME Debounce
					...
					case TOUCH_MODE_SCROLL:
						...
						if (childCount > 0) {
							...
							if (mFirstPosition == 0 && firstChildTop >= contentTop
								...
								// Fling if we have enough velocity and we
								// aren't at a boundary.
								// Since we can potentially overfling more than
								// we can overscroll, don't
								// allow the weird behavior where you can scroll
								// to a boundary then
								// fling further.
								if (Math.abs(initialVelocity) > mMinimumVelocity
										&& !(mFirstPosition == 0 && firstChildTop == contentTop - mOverscrollDistance || mFirstPosition
	...
	public void reclaimViews(final List<View> views) {
		final int childCount = getChildCount();
		final RecyclerListener listener = mRecycler.mRecyclerListener;

		// Reclaim views on screen
		for (int i = 0; i < childCount; i++) {
			final View child = getChildAt(i);
			final AbsListView.LayoutParams lp = (AbsListView.LayoutParams) child.getLayoutParams();
	 ...
	 * the standard scrollbars. Fast scrollers shown in this way will not fade
	 * out and will be a permanent fixture within the list. Best combined with
	 * an inset scroll bar style that will ensure enough padding. This will
	 * enable fast scrolling if it is not already enabled.
	 * 
	...
	public void setFastScrollAlwaysVisible(final boolean alwaysShow) {
		...
		final int scrollBarStyle = getScrollBarStyle();
		setScrollBarStyle(-1);
	...
	public void setFilterText(final String filterText) {
		...
		if (mTextFilterEnabled && !TextUtils.isEmpty(filterText)) {
			createTextFilter(false);
			// This is going to call our listener onTextChanged, but we might
			// not
			// be ready to bring up a window yet
			mTextFilter.setText(filterText);
			mTextFilter.setSelection(filterText.length());
			if (mAdapter instanceof Filterable) {
				// if mPopup is non-null, then onTextChanged will do the
				// filtering
				if (mPopup == null) {
					final Filter f = ((Filterable) mAdapter).getFilter();
					f.filter(filterText);
				}
				// Set filtered to true so we will display the filter window
				// when our main
				// window is ready
				mFiltered = true;
	...
	public void setItemChecked(final int position, final boolean value) {
		...
		if (mChoiceMode == CHOICE_MODE_MULTIPLE || mChoiceMode == CHOICE_MODE_MULTIPLE_MODAL) {
			final boolean oldValue = mCheckStates.get(position);
	 ...
	 * @hide
	 */
	public void setOverScrollEffectPadding(final int leftPadding, final int rightPadding) {
	...
	public boolean showContextMenu(final float x, final float y, final int metaState) {
		...
		if (position != INVALID_POSITION) {
			final long id = mAdapter.getItemId(position);
			final View child = getChildAt(position - mFirstPosition);
			if (child != null) {
	...
	public void smoothScrollBy(final int distance, final int duration) {
		...
		if (distance == 0 || mItemCount == 0 || childCount == 0 || firstPos == 0 && getChildAt(0).getTop() == topLimit
				&& distance < 0 || lastPos == mItemCount - 1 && getChildAt(childCount - 1).getBottom() == bottomLimit
				&& distance > 0) {
			mFlingRunnable.endFling();
			if (mPositionScroller != null) {
				mPositionScroller.stop();
	 ...
	 * 
	 * @param position Scroll to this adapter position.
	 * @param boundPosition Do not scroll if it would move this adapter position
	 *            out of view.
	 */
	 ...
	 * @param offset Desired distance in pixels of <code>position</code> from
	 *            the top of the view when scrolling is finished
	 */
	...
	protected void dispatchDraw(final Canvas canvas) {
		int saveCount = 0;
		final boolean clipToPadding = isClipToPadding();
		if (clipToPadding) {
			saveCount = canvas.save();
			final int scrollX = getScrollX();
			final int scrollY = getScrollY();
			canvas.clipRect(scrollX + getPaddingLeft(), scrollY + getPaddingTop(), scrollX + getRight() - getLeft()
					- getPaddingRight(), scrollY + getBottom() - getTop() - getPaddingBottom());
	...
	protected float getBottomFadingEdgeStrength() {
		...
		if (count == 0)
			...
			if (mFirstPosition + count - 1 < mItemCount - 1) return 1.0f;

			final int bottom = getChildAt(count - 1).getBottom();
			final int height = getHeight();
			final float fadeLength = getVerticalFadingEdgeLength();
			return bottom > height - getPaddingBottom() ? (bottom - height + getPaddingBottom()) / fadeLength
					: fadeEdge;
		}
	}
	...
	protected int getBottomPaddingOffset() {
		return isClipToPadding() ? 0 : getPaddingBottom();
	}
	...
	protected int getLeftPaddingOffset() {
		return isClipToPadding() ? 0 : -getPaddingLeft();
	}

	@Override
	protected int getRightPaddingOffset() {
		return isClipToPadding() ? 0 : getPaddingRight();
	}
	...
	protected float getTopFadingEdgeStrength() {
		...
		if (count == 0)
			...
			if (mFirstPosition > 0) return 1.0f;

			final int top = getChildAt(0).getTop();
			final float fadeLength = getVerticalFadingEdgeLength();
			return top < getPaddingTop() ? -(top - getPaddingTop()) / fadeLength : fadeEdge;
		}
	}

	@Override
	protected int getTopPaddingOffset() {
		return isClipToPadding() ? 0 : -getPaddingTop();
	}

	@Override
	protected void handleDataChanged() {
		final int count = mItemCount;
		final int lastHandledItemCount = mLastHandledItemCount;
		...
		if (count > 0) {
			...
			if (!isInTouchMode()) {
				...
				if (mResurrectToPosition >= 0) return;
			}
	...
	// */
	// @Override
	// protected boolean onConsistencyCheck(final int consistency) {
	 ...
	 */
	private void createTextFilter(final boolean animateEntrance) {
		if (mPopup == null) {
	...
	private void scrollIfNeeded(final int y) {
		...
		if (mTouchMode == TOUCH_MODE_SCROLL) {
			...
			if (y != mLastY) {
				...
				// intercept.
				if (!isDisallowIntercept() && Math.abs(rawDeltaY) > mTouchSlop) {
				...
				if (motionView != null) {
					...
					if (atEdge) {
						...
						final int overscroll = -incrementalDeltaY - (motionViewRealTop - motionViewPrevTop);
			...
			if (y != mLastY) {
				...
				final int newDirection = y > mLastY ? 1 : -1;
	 ...
	 */
	int findClosestMotionRow(final int y) {
		final int childCount = getChildCount();
		if (childCount == 0) return INVALID_POSITION;
	 ...
	 */
	void reportScrollStateChange(final int newState) {
	...
	boolean resurrectSelection() {
		...
		if (toPosition >= firstPosition && toPosition < firstPosition + childCount) {
			...
			final int selectedBottom = selected.getBottom();
	 ...
	 */
	boolean sendToTextFilter(final int keyCode, final int count, final KeyEvent event) {
		if (!acceptFilter()) return false;
	 ...
	 */
	void smoothScrollByOffset(final int position) {
		int index = -1;
		if (position < 0) {
		...
		if (index > -1) {
			...
			if (child != null) {
				...
				if (child.getGlobalVisibleRect(visibleRect)) {
					...
					final int childRectArea = child.getWidth() * child.getHeight();
					final int visibleRectArea = visibleRect.width() * visibleRect.height();
					final float visibleArea = visibleRectArea / (float) childRectArea;
	...
	private static Drawable getDefaultSelector(final Context context) {
		final TypedArray a = context.obtainStyledAttributes(null, new int[] { android.R.attr.listSelector },
				android.R.attr.listViewStyle, android.R.style.Widget_ListView);
	 ...
	 */
	static int getDistance(final Rect source, final Rect dest, final int direction) {
		...
		final int deltaX = dX - sX;
		final int deltaY = dY - sY;
		return deltaY * deltaY + deltaX * deltaX;
	}

	static boolean isScrollingInDirection(final OverScroller scroller, final float xvel, final float yvel) {
		final int dx = scroller.getFinalX() - scroller.getStartX();
		final int dy = scroller.getFinalY() - scroller.getStartY();
		return !scroller.isFinished() && Math.signum(xvel) == Math.signum(dx) && Math.signum(yvel) == Math.signum(dy);
	}

	static View retrieveFromScrap(final ArrayList<View> scrapViews, final int position) {
	...
	private class FlingRunnable implements Runnable {
		...
		private final Runnable mCheckFlywheel = new Runnable() {
			...
			public void run() {
				...
				if (vt == null || activeId == INVALID_POINTER) return;

				vt.computeCurrentVelocity(1000, mMaximumVelocity);
				final float yvel = -vt.getYVelocity(activeId);

				if (Math.abs(yvel) >= mMinimumVelocity && isScrollingInDirection(scroller, 0, yvel)) {
				...
				case TOUCH_MODE_FLING: {
					...
					if (atEnd) {
						if (motionView != null) {
							...
							final int overshoot = -(delta - (motionView.getTop() - oldTop));
					...
					if (scroller.computeScrollOffset()) {
						...
						if (overScrollBy(0, deltaY, 0, scrollY, 0, 0, 0, mOverflingDistance, false)) {
							...
							if (crossDown || crossUp) {
								...
								// Don't flywheel from this; we're just
								// continuing things.
		...
		void edgeReached(final int delta) {
		...
		void start(final int initialVelocity) {
		...
		void startOverfling(final int initialVelocity) {
		...
		void startScroll(final int distance, final int duration) {
	...
	class MultiChoiceModeWrapper implements MultiChoiceModeListener {
		...
		public void setWrapped(final MultiChoiceModeListener wrapped) {
			mWrapped = wrapped;
		}
	}
	...
	class PositionScroller implements Runnable {
		...
		@Override
		public void run() {
			if (mTouchMode != TOUCH_MODE_FLING && mLastSeenPos != INVALID_POSITION) return;
		...
		void start(final int position, final int boundPosition) {
			...
			if (position <= firstPos) {
				...
				if (boundPosFromLast < 1) // Moving would shift our bound
											// position off the screen. Abort.
					return;
				...
				if (boundPosFromFirst < 1) // Moving would shift our bound
											// position off the screen. Abort.
					return;
		...
		void startWithOffset(final int position, final int offset) {
	...
	class RecycleBin {
		...
		public void setViewTypeCount(final int viewTypeCount) {
			if (viewTypeCount < 1) throw new IllegalArgumentException("Can't have a viewTypeCount < 1");
		...
		public boolean shouldRecycleViewType(final int viewType) {
			return viewType >= 0;
		}
		 ...
		 */
		void addScrapView(final View scrap, final int position) {
		 ...
		 */
		void reclaimScrapViews(final List<View> views) {
			if (mViewTypeCount == 1) {
		 ...
		 */
		void setCacheColorHint(final int color) {
			if (mViewTypeCount == 1) {
				final ArrayList<View> scrap = mCurrentScrap;
				final int scrapCount = scrap.size();
				for (int i = 0; i < scrapCount; i++) {
