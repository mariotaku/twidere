...
class FastScroller {
	...
	private static final int[] ATTRS = new int[] { android.R.attr.fastScrollTextColor,
	...
	public FastScroller(final Context context, final AbsListView listView) {
		mList = listView;
		init(context);
	}
	...
	public void draw(final Canvas canvas) {
		...
		if (mState == STATE_NONE) // No need to draw anything
			return;
		...
		if (mTrackDrawable != null) {
			...
			mTrackDrawable.setBounds(trackLeft, halfThumbHeight, trackLeft + trackWidth, mList.getHeight()
					- halfThumbHeight);
		...
		if (mState == STATE_DRAGGING && mDrawOverlay) {
			...
			canvas.drawText(mSectionText, (int) (rectF.left + rectF.right) / 2 - hOff, (int) (rectF.bottom + rectF.top)
					/ 2 + mOverlaySize / 4 - descent - vOff, paint);
	...
	private int getThumbPositionForListPosition(int firstVisibleItem, final int visibleItemCount, int totalItemCount) {
	...
	private void init(final Context context) {
		...
		final int textColorNormal = textColor.getDefaultColor();
	...
	private void refreshDrawableState() {
		final int[] state = mState == STATE_DRAGGING ? PRESSED_STATES : DEFAULT_STATES;
	...
	private void useThumbDrawable(final Context context, final Drawable drawable) {
	...
	boolean isPointInside(final float x, final float y) {
	...
	boolean onInterceptTouchEvent(final MotionEvent ev) {
	...
	void onItemCountChanged(final int oldCount, final int newCount) {
		if (mAlwaysShow) {
			mLongList = true;
		}
	}
	...
	void onScroll(final AbsListView view, final int firstVisibleItem, final int visibleItemCount,
		...
		mScrollCompleted = true;
		if (firstVisibleItem == mVisibleItem) return;
	...
	boolean onTouchEvent(final MotionEvent me) {
		...
		if (action == MotionEvent.ACTION_DOWN) {
			...
			if (mState == STATE_DRAGGING) {
				...
				if (Math.abs(mThumbY - newThumbY) < 2) return true;
				mThumbY = newThumbY;
	...
	void scrollTo(final float position) {
		final int count = mList.getCount();
		mScrollCompleted = false;
		final float fThreshold = 1.0f / count / 8;
		...
		if (sections != null && sections.length > 1) {
			...
			final int exactSection = section;
	...
	public class ScrollFade implements Runnable {
		...
		int getAlpha() {
			if (getState() != STATE_EXIT) return ALPHA_MAX;
			int alpha;
