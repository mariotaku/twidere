...
public class ListView extends AbsListView {
	...
	public ListView(final Context context, final AttributeSet attrs, final int defStyle) {
		...
		final CharSequence[] entries = a.getTextArray(styleable.ListView_entries);
		if (entries != null) {
			setAdapter(new ArrayAdapter<CharSequence>(context, android.R.layout.simple_list_item_1, entries));
		}

		final Drawable d = a.getDrawable(styleable.ListView_divider);
		if (d != null) {
	...
	public boolean isOpaque() {
		final boolean retValue = mCachingActive && mIsCacheColorOpaque && mDividerIsOpaque && hasOpaqueScrollbars()
				|| super.isOpaque();
		if (retValue) {
			...
			if (first == null || first.getTop() > listTop) return false;
			...
			if (last == null || last.getBottom() < listBottom) return false;
		}
		return retValue;
	}
	 ...
	 */
	public void setSelectionFromTop(int position, final int y) {
	 ...
	 * 
	 * @param offset The amount to offset from the adapter position to scroll
	 *            to.
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see android.view.View#findViewById(int) First look in our children, then
	 * in any header and footer views that may be scrolled off.
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see android.view.View#findViewWithTag(Object) First look in our
	 * children, then in any header and footer views that may be scrolled off.
	 */
	@Override
	...
	protected boolean hasOpaqueScrollbars() {
		return !isVerticalScrollBarEnabled() && !isHorizontalScrollBarEnabled()
				|| getScrollBarStyle() == SCROLLBARS_INSIDE_OVERLAY;
	}
	 ...
	 */
	private int amountToScroll(final int direction, final int nextSelectedPosition) {
		...
		if (direction == View.FOCUS_DOWN) {
			...
			if (viewToMakeVisible.getBottom() <= goalBottom) // item is fully
																// visible.
				return 0;

			if (nextSelectedPosition != INVALID_POSITION
					&& goalBottom - viewToMakeVisible.getTop() >= getMaxScrollAmount()) // item
																						// already
																						// has
																						// enough
																						// of
			...
			if (viewToMakeVisible.getTop() >= goalTop) // item is fully visible.
				return 0;

			if (nextSelectedPosition != INVALID_POSITION
					&& viewToMakeVisible.getBottom() - goalTop >= getMaxScrollAmount()) // item
																						// already
																						// has
																						// enough
																						// of
	 ...
	 * @param positionOfNewFocus The position of the list item containing
	 *            newFocus
	 * @return The amount to scroll. Note: this is always positive! Direction
	 *         needs to be taken into account when actually scrolling.
	 */
	private int amountToScrollToNewFocus(final int direction, final View newFocus, final int positionOfNewFocus) {
	...
	private ArrowScrollFocusResult arrowScrollFocused(final int direction) {
		...
		if (selectedView != null && selectedView.hasFocus()) {
			final View oldFocus = selectedView.findFocus();
			...
			if (direction == View.FOCUS_DOWN) {
				...
				final int ySearchPoint = selectedView != null && selectedView.getTop() > listTop ? selectedView
						.getTop() : listTop;
				...
				final int ySearchPoint = selectedView != null && selectedView.getBottom() < listBottom ? selectedView
		...
		if (newFocus != null) {
			...
			if (mSelectedPosition != INVALID_POSITION && positionOfNewFocus != mSelectedPosition) {
				...
				if (selectablePosition != INVALID_POSITION
						&& (direction == View.FOCUS_DOWN && selectablePosition < positionOfNewFocus || direction == View.FOCUS_UP
								&& selectablePosition > positionOfNewFocus)) return null;
			}

			final int focusScroll = amountToScrollToNewFocus(direction, newFocus, positionOfNewFocus);
	 ...
	 */
	private boolean arrowScrollImpl(final int direction) {
	...
	private boolean commonKey(final int keyCode, int count, final KeyEvent event) {
		if (mAdapter == null || !mIsAttached) return false;
		...
		if (action != KeyEvent.ACTION_UP) {
			switch (keyCode) {
				...
				case KeyEvent.KEYCODE_TAB:
					...
					// asymmetry in TAB navigation order unless the list
					// selection
					// always reverts to the top or bottom when receiving TAB
					// focus from
					// another widget. Leaving this behavior disabled for now
					// but
					// perhaps it should be configurable (and more
					// comprehensive).
					// if (false) {
					// if (event.hasNoModifiers()) {
					// handled = resurrectSelectionIfNeeded() ||
	 ...
	 */
	private int distanceToView(final View descendant) {
		int distance = 0;
	 ...
	 */
	private void fillAboveAndBelow(final View sel, final int position) {
		final int dividerHeight = mDividerHeight;
		if (!mStackFromBottom) {
	...
	private View fillDown(int pos, int nextTop) {
		...
		while (nextTop < end && pos < mItemCount) {
			...
			final boolean selected = pos == mSelectedPosition;
			final View child = makeAndAddView(pos, nextTop, true, mListPadding.left, selected);
	 ...
	 */
	private View fillFromMiddle(final int childrenTop, final int childrenBottom) {
		final int height = childrenBottom - childrenTop;

		final int position = reconcileSelectedPosition();

		final View sel = makeAndAddView(position, childrenTop, true, mListPadding.left, true);
		mFirstPosition = position;

		final int selHeight = sel.getMeasuredHeight();
		if (selHeight <= height) {
	 ...
	 */
	private View fillFromSelection(final int selectedTop, final int childrenTop, final int childrenBottom) {
	...
	private View fillSpecific(final int position, final int top) {
		...
		final View temp = makeAndAddView(position, top, true, mListPadding.left, tempIsSelected);
	...
	private View fillUp(int pos, int nextBottom) {
		...
		while (nextBottom > end && pos >= 0) {
			// is this the selected item?
			final boolean selected = pos == mSelectedPosition;
			final View child = makeAndAddView(pos, nextBottom, false, mListPadding.left, selected);
	 ...
	 */
	private int getBottomSelectionPixel(final int childrenBottom, final int fadingEdgeLength, final int selectedPosition) {
	 ...
	 */
	private int getTopSelectionPixel(final int childrenTop, final int fadingEdgeLength, final int selectedPosition) {
	 ...
	 */
	private boolean handleHorizontalFocusWithinListItem(final int direction) {
		...
		if (mItemsCanFocus && numChildren > 0 && mSelectedPosition != INVALID_POSITION) {
			...
			if (selectedView != null && selectedView.hasFocus() && selectedView instanceof ViewGroup) {
				...
				if (nextFocus != null) {
					...
					if (nextFocus.requestFocus(direction, mTempRect)) return true;
				}
				...
				if (globalNextFocus != null) return isViewAncestorOf(globalNextFocus, this);
			}
		}
		return false;
	}
	 ...
	 * 
	 * @param selectedView The currently selected view (before changing
	 *            selection). should be <code>null</code> if there was no
	 *            previous selection.
	 * @param direction Either {@link android.view.View#FOCUS_UP} or
	 *            {@link android.view.View#FOCUS_DOWN}.
	 * @param newSelectedPosition The position of the next selection.
	 * @param newFocusAssigned whether new focus was assigned. This matters
	 *            because when something has focus, we don't want to show
	 *            selection (ugh).
	 */
	private void handleNewSelectionChange(final View selectedView, final int direction, final int newSelectedPosition,
			final boolean newFocusAssigned) {
	 ...
	 */
	private boolean isViewAncestorOf(final View child, final View parent) {
	 ...
	 */
	private int lookForSelectablePositionOnScreen(final int direction) {
		final int firstPosition = mFirstPosition;
		if (direction == View.FOCUS_DOWN) {
			int startPos = mSelectedPosition != INVALID_POSITION ? mSelectedPosition + 1 : firstPosition;
			if (startPos >= mAdapter.getCount()) return INVALID_POSITION;
			...
			int startPos = mSelectedPosition != INVALID_POSITION ? mSelectedPosition - 1 : firstPosition
					+ getChildCount() - 1;
			if (startPos < 0 || startPos >= mAdapter.getCount()) return INVALID_POSITION;
			if (startPos > last) {
				startPos = last;
			}
	 ...
	 */
	private View makeAndAddView(final int position, final int y, final boolean flow, final int childrenLeft,
	 ...
	 */
	private void measureAndAdjustDown(final View child, final int childIndex, final int numChildren) {
		final int oldHeight = child.getHeight();
		measureItem(child);
	 ...
	 * @param child The child.
	 */
	private void measureItem(final View child) {
	 ...
	 * @return The view that currently has selection
	 */
	private View moveSelection(View oldSel, final View newSel, final int delta, final int childrenTop,
		...
		if (delta > 0) {
			...
			if (sel.getBottom() > bottomSelectionPixel) {
				...
				final int halfVerticalSpace = (childrenBottom - childrenTop) / 2;
			...
			final int oldTop = oldSel.getTop();
			...
			if (oldTop < childrenTop) {
				...
				final int newBottom = sel.getBottom();
	 ...
	 */
	private int positionOfNewFocus(final View newFocus) {
		final int numChildren = getChildCount();
		for (int i = 0; i < numChildren; i++) {
			final View child = getChildAt(i);
			if (isViewAncestorOf(newFocus, child)) return mFirstPosition + i;
		}
	 ...
	 * @param child The child.
	 */
	private void relayoutMeasuredItem(final View child) {
	...
	private void scrollListItemsBy(final int amount) {
		...
		if (amount < 0) {
			...
			while (first.getBottom() < listTop) {
				final AbsListView.LayoutParams layoutParams = (LayoutParams) first.getLayoutParams();
			...
			while (last.getTop() > listBottom) {
				final AbsListView.LayoutParams layoutParams = (LayoutParams) last.getLayoutParams();
				if (recycleBin.shouldRecycleViewType(layoutParams.viewType)) {
	 ...
	 */
	boolean arrowScroll(final int direction) {
		try {
			mInLayout = true;
	 ...
	 */
	@Override
	void fillGap(final boolean down) {
		final int count = getChildCount();
		if (down) {
			int paddingTop = 0;
			if (isClipToPadding()) {
	 ...
	 */
	View findViewInHeadersOrFooters(final ArrayList<FixedViewInfo> where, final int id) {
		if (where != null) {
			final int len = where.size();
			View v;
	 ...
	 * (non-Javadoc)
	 * 
	 * Look in the passed in list of headers or footers for the view with the
	 * tag.
	 */
	View findViewWithTagInHeadersOrFooters(final ArrayList<FixedViewInfo> where, final Object tag) {
		if (where != null) {
			final int len = where.size();
			View v;
	 ...
	 * @return whether selection was moved
	 */
	boolean fullScroll(final int direction) {
		boolean moved = false;
		if (direction == FOCUS_UP) {
			...
			if (mSelectedPosition < mItemCount - 1) {
				final int position = lookForSelectablePosition(mItemCount - 1, true);
				if (position >= 0) {
	 ...
	 */
	@Override
	int lookForSelectablePosition(int position, final boolean lookDown) {
		final ListAdapter adapter = mAdapter;
	 ...
	 * @return whether selection was moved
	 */
	boolean pageScroll(final int direction) {
		int nextPage = -1;
		boolean down = false;
		...
		if (nextPage >= 0) {
			final int position = lookForSelectablePosition(nextPage, down);
			if (position >= 0) {
				mLayoutMode = LAYOUT_SPECIFIC;
	 ...
	 */
	@Override
	void setSelectionInt(final int position) {
		setNextSelectedPositionInt(position);
	...
	static private class ArrowScrollFocusResult {
		 ...
		 * values.
		 */
		void populate(final int selectedPosition, final int amountToScroll) {
	...
	private class FocusSelector implements Runnable {
		...
		public FocusSelector setup(final int position, final int top) {
			mPosition = position;
			mPositionTop = top;
			return this;
		}
