...
public class DraftsActivity extends TwidereSwipeBackActivity implements LoaderCallbacks<Cursor>,
        OnItemClickListener,
    ...
    private void sendDraft(final DraftItem draft) {
        ...
        mResolver.delete(Drafts.CONTENT_URI, Drafts._ID + " = " + draft._id, null);
        mTwitterWrapper
                .updateStatus(draft.account_ids, draft.text, draft.location, uri,
                        draft.in_reply_to_status_id,
                        draft.is_possibly_sensitive,
                        draft.attached_image_type == ATTACHED_IMAGE_TYPE_PHOTO);
    }
    ...
    static class DraftsAdapter extends SimpleCursorAdapter {
        ...
        public void bindView(final View view, final Context context, final Cursor cursor) {
            ...
            image_preview_container.setVisibility(TextUtils.isEmpty(image_uri) ? View.GONE
                    : View.VISIBLE);
            if (!TextUtils.isEmpty(image_uri)
                    && !image_uri.equals(mImageLoadingHandler.getLoadingUri(image))) {
