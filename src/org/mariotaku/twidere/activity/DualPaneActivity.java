...
public class DualPaneActivity extends BaseSupportActivity implements OnBackStackChangedListener {
    ...
    public final boolean isDualPaneMode() {
        return findViewById(PANE_LEFT) instanceof ViewGroup
                && findViewById(PANE_RIGHT) instanceof ViewGroup;
    }
    ...
    public void onBackStackChanged() {
        if (isDualPaneMode()) {
            ...
            final boolean right_pane_used = right_pane_fragment != null
                    && right_pane_fragment.isAdded();
            if (count > 0) {
            ...
            if (main_view != null) {
                ...
                if (main_view.getVisibility() != visibility) {
                    final Animation anim = AnimationUtils.loadAnimation(this,
                            left_pane_used ? android.R.anim.fade_out
    ...
    public final void showAtPane(final int pane, final Fragment fragment,
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        mDualPaneInPortrait = mPreferences.getBoolean(PREFERENCE_KEY_DUAL_PANE_IN_PORTRAIT,
                is_large_screen);
        mDualPaneInLandscape = mPreferences.getBoolean(PREFERENCE_KEY_DUAL_PANE_IN_LANDSCAPE,
                is_large_screen);
        switch (orientation) {
    ...
    protected void onStart() {
        ...
        final Resources res = getResources();
        final boolean is_large_screen = res.getBoolean(R.bool.is_large_screen);
        final boolean dual_pane_in_portrait = mPreferences.getBoolean(
                PREFERENCE_KEY_DUAL_PANE_IN_PORTRAIT,
                is_large_screen);
        final boolean dual_pane_in_landscape = mPreferences.getBoolean(
                PREFERENCE_KEY_DUAL_PANE_IN_LANDSCAPE,
                is_large_screen);
        final int orientation = res.getConfiguration().orientation;
        switch (orientation) {
            case Configuration.ORIENTATION_LANDSCAPE:
