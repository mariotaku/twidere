...
public class EditAPIActivity extends BaseSupportDialogActivity implements TwitterConstants,
        OnCheckedChangeListener,
        OnClickListener {

    private EditText mEditRestBaseURL, mEditSigningRESTBaseURL, mEditOAuthBaseURL,
            mEditSigningOAuthBaseURL;
    ...
    public void onClick(final View v) {
        final SharedPreferences pref = getSharedPreferences(SHARED_PREFERENCES_NAME,
                Context.MODE_PRIVATE);
        final String consumer_key = getNonEmptyString(pref, PREFERENCE_KEY_CONSUMER_KEY,
        ...
        final String rest_base_url = getNonEmptyString(pref, PREFERENCE_KEY_REST_BASE_URL,
                DEFAULT_REST_BASE_URL);
        final String oauth_base_url = getNonEmptyString(pref, PREFERENCE_KEY_OAUTH_BASE_URL,
                DEFAULT_OAUTH_BASE_URL);
        final String signing_rest_base_url = getNonEmptyString(pref,
                PREFERENCE_KEY_SIGNING_REST_BASE_URL,
                DEFAULT_SIGNING_REST_BASE_URL);
        final String signing_oauth_base_url = getNonEmptyString(pref,
        ...
        switch (v.getId()) {
            case R.id.save: {
                ...
                intent.putExtra(Accounts.REST_BASE_URL, isEmpty(mRestBaseURL) ? rest_base_url
                        : mRestBaseURL);
                intent.putExtra(Accounts.OAUTH_BASE_URL, isEmpty(mOAuthBaseURL) ? oauth_base_url
                        : mOAuthBaseURL);
                intent.putExtra(Accounts.SIGNING_REST_BASE_URL,
                        isEmpty(mSigningRestBaseURL) ? signing_rest_base_url
                                : mSigningRestBaseURL);
                intent.putExtra(Accounts.SIGNING_OAUTH_BASE_URL,
                        isEmpty(mSigningOAuthBaseURL) ? signing_oauth_base_url
                                : mSigningOAuthBaseURL);
                intent.putExtra(Accounts.CONSUMER_KEY, isEmpty(mConsumerKey) ? consumer_key
                        : mConsumerKey);
                intent.putExtra(Accounts.CONSUMER_SECRET,
                        isEmpty(mConsumerSecret) ? consumer_secret : mConsumerSecret);
            ...
            case R.id.advanced_api_config_label: {
                final View stub_view = mAdvancedAPIConfigContainer
                        .findViewById(R.id.stub_advanced_api_config);
                final View inflated_view = mAdvancedAPIConfigContainer
                        .findViewById(R.id.advanced_api_config);
                if (stub_view != null) {
                    stub_view.setVisibility(View.VISIBLE);
                    mAdvancedAPIConfigLabel.setCompoundDrawablesWithIntrinsicBounds(
                            R.drawable.expander_open_holo, 0,
                            0, 0);
                    mEditSigningRESTBaseURL = (EditText) mAdvancedAPIConfigContainer
                            .findViewById(R.id.signing_rest_base_url);
                    mEditOAuthBaseURL = (EditText) mAdvancedAPIConfigContainer
                            .findViewById(R.id.oauth_base_url);
                    mEditSigningOAuthBaseURL = (EditText) mAdvancedAPIConfigContainer
                            .findViewById(R.id.signing_oauth_base_url);
                    mEditConsumerKey = (EditText) mAdvancedAPIConfigContainer
                            .findViewById(R.id.consumer_key);
                    mEditConsumerSecret = (EditText) mAdvancedAPIConfigContainer
                            .findViewById(R.id.consumer_secret);

                    mEditSigningRESTBaseURL
                            .setText(isEmpty(mSigningRestBaseURL) ? DEFAULT_SIGNING_REST_BASE_URL
                                    : mSigningRestBaseURL);
                    mEditOAuthBaseURL.setText(isEmpty(mOAuthBaseURL) ? DEFAULT_OAUTH_BASE_URL
                            : mOAuthBaseURL);
                    mEditSigningOAuthBaseURL
                            .setText(isEmpty(mSigningOAuthBaseURL) ? DEFAULT_SIGNING_OAUTH_BASE_URL
                                    : mSigningOAuthBaseURL);
                    mEditConsumerKey.setText(isEmpty(mConsumerKey) ? consumer_key : mConsumerKey);
                    mEditConsumerSecret.setText(isEmpty(mConsumerSecret) ? consumer_secret
                            : mConsumerSecret);
                    ...
                    mAdvancedAPIConfigLabel.setCompoundDrawablesWithIntrinsicBounds(compound_res,
                            0, 0, 0);
