...
public class CursorStatusesAdapter extends SimpleCursorAdapter implements IStatusesAdapter<Cursor>,
        OnClickListener {
    ...
    private boolean mDisplayProfileImage, mDisplayImagePreview, mShowAccountColor, mGapDisallowed,
            mMentionsHighlightDisabled, mDisplaySensitiveContents, mIndicateMyStatusDisabled,
            mLinkHighlightingEnabled,
            mIsLastItemFiltered, mFiltersEnabled = true;
    private float mTextSize;
    private int mNameDisplayOption, mLinkHighlightStyle;
    private boolean mFilterIgnoreSource, mFilterIgnoreScreenName, mFilterIgnoreTextHtml,
            mFilterIgnoreTextPlain;
    ...
    public void bindView(final View view, final Context context, final Cursor cursor) {
        ...
        if (!show_gap) {
            ...
            final String retweeted_by_screen_name = cursor
                    .getString(mIndices.retweeted_by_screen_name);
            final String text = mLinkHighlightingEnabled ? cursor.getString(mIndices.text_html)
                    : cursor
            ...
            final String in_reply_to_screen_name = cursor
                    .getString(mIndices.in_reply_to_screen_name);
            ...
            final boolean is_mention = TextUtils.isEmpty(text)
                    || TextUtils.isEmpty(account_screen_name) ? false : text
                    .toLowerCase(Locale.US).contains(
                            '@' + account_screen_name.toLowerCase(Locale.US));
            final boolean is_my_status = account_id == user_id;

            holder.setUserColor(getUserColor(mContext, user_id));
            holder.setHighlightColor(getStatusBackground(mMentionsHighlightDisabled ? false
                    : is_mention, is_favorite,
                    is_retweet));
            ...
            holder.name.setText(TextUtils.isEmpty(nick) ? name : context.getString(
                    R.string.name_with_nickname, name,
                    nick));
            ...
            if (has_preview) {
                ...
                } else if (!image_preview_url.equals(mImageLoadingHandler
                        .getLoadingUri(holder.image_preview))) {
                    holder.image_preview.setBackgroundResource(0);
                    mImageLoader.displayPreviewImage(holder.image_preview, image_preview_url,
    ...
    public void onClick(final View view) {
        ...
        switch (view.getId()) {
            case R.id.image_preview: {
                ...
                if (status == null)
                    return;
                final PreviewImage spec = PreviewImage
                        .getAllAvailableImage(status.image_original_url);
                if (spec != null) {
                    openImage(mContext, spec.image_full_url, spec.image_original_url,
                            status.is_possibly_sensitive);
                } else {
                    openImage(mContext, status.image_original_url, null,
                            status.is_possibly_sensitive);
                }
                break;
            }
            ...
            case R.id.profile_image: {
                ...
                if (mContext instanceof Activity) {
                    openUserProfile((Activity) mContext, status.account_id, status.user_id,
    ...
    public void setDisplayImagePreview(final boolean display) {
        if (display == mDisplayImagePreview)
            return;
    ...
    @Override
    public void setIgnoredFilterFields(final boolean text_plain, final boolean text_html,
            final boolean screen_name,
    ...
    private void rebuildFilterInfo() {
        ...
        if (cursor != null && mIndices != null && cursor.getCount() > 0) {
            if (cursor.getCount() > 0) {
                cursor.moveToLast();
                final String text_plain = mFilterIgnoreTextPlain ? null : cursor
                        .getString(mIndices.text_plain);
                final String text_html = mFilterIgnoreTextHtml ? null : cursor
                        .getString(mIndices.text_html);
                final String screen_name = mFilterIgnoreScreenName ? null : cursor
                        .getString(mIndices.user_screen_name);
                final String source = mFilterIgnoreSource ? null : cursor
                        .getString(mIndices.source);
                mIsLastItemFiltered = isFiltered(mDatabase, text_plain, text_html, screen_name,
                        source);
            } else {
