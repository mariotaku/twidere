...
public class ParcelableActivitiesAdapter extends ArrayAdapter<ParcelableActivity> implements
        IBaseAdapter {
    ...
    public void setNameDisplayOption(final String option) {
        ...
        if (option_int == mNameDisplayOption)
            return;
    ...
    private String getName(final ParcelableStatus status) {
        if (status == null)
            return null;
        return mNameDisplayOption == NAME_DISPLAY_OPTION_CODE_SCREEN_NAME ? status.user_screen_name
                : status.user_name;
    }
    ...
    private String getName(final ParcelableUser user) {
        if (user == null)
            return null;
        return mNameDisplayOption == NAME_DISPLAY_OPTION_CODE_SCREEN_NAME ? user.screen_name
                : user.name;
    }
    ...
    private boolean isMyActivity(final ParcelableActivity activity) {
        ...
        final int target_object_statuses_length = target_object_statuses != null ? target_object_statuses.length
                : 0;
        if (activity.action == ParcelableActivity.ACTION_FAVORITE)
            ...
            return target_object_statuses_length > 0
                    && target_object_statuses[0].user_id == activity.account_id;
    ...
    private void setUserProfileImages(final ActivityViewHolder holder,
            final ParcelableStatus[] statuses) {
        final int length = statuses != null ? Math.min(holder.activity_profile_images.length,
                statuses.length) : 0;
        final boolean should_display_images = mDisplayProfileImage && length > 0;
        holder.activity_profile_image_container.setVisibility(should_display_images ? View.VISIBLE
                : View.GONE);
    ...
    private void setUserProfileImages(final ActivityViewHolder holder, final ParcelableUser[] users) {
        final int length = users != null ? Math.min(holder.activity_profile_images.length,
                users.length) : 0;
        final boolean should_display_images = mDisplayProfileImage && length > 0;
        holder.activity_profile_image_container.setVisibility(should_display_images ? View.VISIBLE
                : View.GONE);
    ...
    private void showActivityAboutMe(final ActivityViewHolder holder,
            final ParcelableActivity activity) {
        if (activity == null)
            return;
        ...
        if (sources_length > 0) {
            ...
            switch (action) {
                case ParcelableActivity.ACTION_FAVORITE: {
                    ...
                    if (sources_length == 1) {
                        holder.title.setText(mContext.getString(
                                R.string.activity_about_me_favorite, source_name));
                    } else {
                        holder.title.setText(mContext.getString(
                                R.string.activity_about_me_favorite_multi, source_name,
                                sources_length - 1));
                    }
                ...
                case ParcelableActivity.ACTION_FOLLOW: {
                    ...
                    if (sources_length == 1) {
                        holder.title.setText(mContext.getString(R.string.activity_about_me_follow,
                                source_name));
                    } else {
                        holder.title.setText(mContext.getString(
                                R.string.activity_about_me_follow_multi, source_name,
                                sources_length - 1));
                    }
                    setUserProfileImages(holder, sources);
                    break;
                ...
                case ParcelableActivity.ACTION_RETWEET: {
                    ...
                    if (sources_length == 1) {
                        holder.title.setText(mContext.getString(R.string.activity_about_me_retweet,
                                source_name));
                    } else {
                        holder.title.setText(mContext.getString(
                                R.string.activity_about_me_retweet_multi, source_name,
                                sources_length - 1));
                    }
                    setUserProfileImages(holder, sources);
                    break;
                ...
                case ParcelableActivity.ACTION_LIST_MEMBER_ADDED: {
                    ...
                    if (sources_length == 1) {
                        holder.title.setText(mContext.getString(
                                R.string.activity_about_me_list_member_added,
                                source_name));
                    } else {
                        holder.title.setText(mContext.getString(
    ...
    private void showActivityByFriends(final ActivityViewHolder holder,
            final ParcelableActivity activity) {
        if (activity == null)
            return;
        final ParcelableUser[] sources = activity.sources;
        ...
        final int target_object_user_lists_length = target_object_user_lists != null ? target_object_user_lists.length
                : 0;
        final int target_user_lists_length = target_user_lists != null ? target_user_lists.length
                : 0;
        final int action = activity.action;
        holder.profile_image.setVisibility(mDisplayProfileImage ? View.VISIBLE : View.GONE);
        if (sources_length > 0) {
            ...
            switch (action) {
                case ParcelableActivity.ACTION_FAVORITE: {
                    ...
                    if (target_statuses_length == 1) {
                        ...
                        holder.title.setText(mContext.getString(
                                R.string.activity_by_friends_favorite,
                                first_source_name, getName(first_status)));
                    } else {
                        holder.text.setVisibility(View.GONE);
                        holder.title.setText(mContext.getString(
                                R.string.activity_by_friends_favorite_multi,
                                first_source_name, getName(first_status),
                                target_statuses_length - 1));
                    }
                    setProfileImage(holder, first_source);
                ...
                case ParcelableActivity.ACTION_FOLLOW: {
                    holder.text.setVisibility(View.GONE);
                    if (target_users_length == 0)
                        return;
                    if (target_users_length == 1) {
                        holder.title.setText(mContext.getString(
                                R.string.activity_by_friends_follow, first_source_name,
                                getName(target_users[0])));
                    } else {
                        holder.title.setText(mContext.getString(
                                R.string.activity_by_friends_follow_multi,
                                first_source_name, getName(target_users[0]),
                                target_users_length - 1));
                    }
                    setProfileImage(holder, first_source);
                ...
                case ParcelableActivity.ACTION_RETWEET: {
                    ...
                    if (sources_length == 1) {
                        holder.title.setText(mContext.getString(
                                R.string.activity_by_friends_retweet,
                                first_source_name, getName(target_statuses[0])));
                    } else {
                        holder.title.setText(mContext.getString(
                                R.string.activity_about_me_retweet_multi,
                ...
                case ParcelableActivity.ACTION_LIST_MEMBER_ADDED: {
                    ...
                    if (target_object_user_lists_length == 1) {
                        holder.title.setText(mContext.getString(
                                R.string.activity_by_friends_list_member_added,
                                first_source_name, getName(target_users[0])));
                    } else {
                        holder.title.setText(mContext.getString(
                                R.string.activity_about_me_list_member_added_multi,
