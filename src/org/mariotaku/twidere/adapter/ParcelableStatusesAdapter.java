    ...
    public View getView(final int position, final View convertView, final ViewGroup parent) {
        ...
        if (!show_gap) {
            ...
            final boolean is_mention = !TextUtils.isEmpty(status.text_plain)
                    && status.text_plain.toLowerCase().contains(
            ...
            if (mLinkHighlightingEnabled) {
                mLinkify.applyUserProfileLink(holder.name, status.account_id, status.user_id,
                        status.user_screen_name);
                mLinkify.applyUserProfileLink(holder.screen_name, status.account_id,
                        status.user_id,
                        status.user_screen_name);
            ...
            holder.setStatusType(status.is_favorite, isValidLocation(status.location),
                    status.has_media,
                    status.is_possibly_sensitive);
            holder.setIsReplyRetweet(status.in_reply_to_status_id > 0, status.is_retweet);
            if (status.is_retweet) {
                holder.setRetweetedBy(status.retweet_count, status.retweeted_by_name,
            ...
            if (has_preview) {
                ...
                } else if (!status.image_preview_url.equals(mImageLoadingHandler
