...
public class SourceAutoCompleteAdapter extends SimpleCursorAdapter implements Constants {
    ...
    private static final String[] COLUMNS = new String[] {
            CachedStatuses._ID, CachedStatuses.SOURCE
    };
    ...
    public CharSequence convertToString(final Cursor cursor) {
        if (isCursorClosed() || mSourceIdx == -1)
            return null;
    ...
    public Cursor runQueryOnBackgroundThread(final CharSequence constraint) {
        final String constraint_escaped = constraint != null ? constraint.toString().replaceAll(
                "_", "^_") : null;
        final String selection = constraint != null ? CachedStatuses.SOURCE
                + " LIKE '%\">'||?||'%</a>' ESCAPE '^'"
                : null;
        final String[] selectionArgs = constraint != null ? new String[] {
            constraint_escaped
        } : null;
