...
public abstract class BaseActivitiesListFragment extends BasePullToRefreshListFragment implements
    ...
    @Override
    public void onLoadFinished(final Loader<List<ParcelableActivity>> loader,
    ...
    public void onResume() {
        ...
        final boolean show_absolute_time = mPreferences.getBoolean(
                PREFERENCE_KEY_SHOW_ABSOLUTE_TIME, false);
    ...
    protected void saveActivities() {
        if (getActivity() == null || getView() == null || mIsActivitiesSaved)
            return;
    ...
    protected final boolean saveActivitiesInternal() {
        ...
        try {
            ...
            final List<ParcelableActivity> activities = data.subList(0,
                    Math.min(items_limit, data.size()));
            final File file = JSONSerializer.getSerializationFile(getActivity(),
                    getSavedActivitiesFileArgs());
            JSONSerializer.toFile(file,
                    activities.toArray(new ParcelableActivity[activities.size()]));
