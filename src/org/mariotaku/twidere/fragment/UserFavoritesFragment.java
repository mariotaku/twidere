...
public class UserFavoritesFragment extends ParcelableStatusesListFragment {
    ...
    private final BroadcastReceiver mStateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(final Context context, final Intent intent) {
            ...
            if (BROADCAST_FAVORITE_CHANGED.equals(action)) {
                ...
                if (status == null)
                    return;
                if ((isSameAccount(context, status.account_id, mUserId) || isSameAccount(context,
                        status.account_id,
                        mUserScreenName))
                        && status.id > 0
    ...
    public Loader<List<ParcelableStatus>> newLoaderInstance(final Context context, final Bundle args) {
        ...
        return new UserFavoritesLoader(getActivity(), account_id, user_id, screen_name, max_id,
                since_id, getData(),
    ...
    protected String[] getSavedStatusesFileArgs() {
        ...
        final String screen_name = args.getString(INTENT_KEY_SCREEN_NAME);
        return new String[] {
                AUTHORITY_USER_FAVORITES, "account" + account_id, "user" + user_id,
                "name" + screen_name
        };
    }
