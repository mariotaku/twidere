    ...
    public SingleResponse<ParcelableUser> loadInBackground() {
        if (!omit_intent_extra && extras != null) {
            ...
            if (user != null) {
                ...
                resolver.delete(CachedUsers.CONTENT_URI, CachedUsers.USER_ID + " = " + user.id,
                        null);
        ...
        if (load_from_cache) {
            final String where = CachedUsers.USER_ID + " = " + user_id + " OR "
                    + CachedUsers.SCREEN_NAME + " = '"
                    + screen_name + "'";
            final Cursor cur = resolver.query(CachedUsers.CONTENT_URI, CachedUsers.COLUMNS, where,
                    null, null);
            final int count = cur.getCount();
            try {
                if (count > 0) {
                    cur.moveToFirst();
                    return new SingleResponse<ParcelableUser>(new ParcelableUser(cur, account_id),
                            null);
                }
            } finally {
                cur.close();
        ...
        try {
            ...
            final String where = CachedUsers.USER_ID + " = " + user.getId() + " OR "
                    + CachedUsers.SCREEN_NAME + " = ?";
            resolver.delete(CachedUsers.CONTENT_URI, where, new String[] {
                user.getScreenName()
            });
            resolver.insert(CachedUsers.CONTENT_URI, values);
            return new SingleResponse<ParcelableUser>(new ParcelableUser(user, account_id,
