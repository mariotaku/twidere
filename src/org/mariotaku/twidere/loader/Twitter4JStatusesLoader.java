...
public abstract class Twitter4JStatusesLoader extends ParcelableStatusesLoader {
    ...
    public Twitter4JStatusesLoader(final Context context, final long account_id, final long max_id,
            final long since_id, final List<ParcelableStatus> data,
            final String[] saved_statuses_args,
    ...
    public List<ParcelableStatus> loadInBackground() {
        ...
        if (isFirstLoad() && getTabPosition() >= 0) {
            try {
                final File file = JSONSerializer.getSerializationFile(getContext(),
                        mSavedStatusesFileArgs);
        ...
        if (statuses != null) {
            ...
            final boolean insert_gap = min_status_id > 0 && load_item_limit <= statuses.size()
                    && data.size() > 0;
            mHandler.post(CacheUsersStatusesTask.getRunnable(context, new StatusListResponse(
                    mAccountId, statuses)));
            for (final Status status : statuses) {
                final long id = status.getId();
                deleteStatus(id);
                data.add(new ParcelableStatus(status, mAccountId,
                        min_status_id == id && insert_gap, mHiResProfileImage));
            }
        }
        try {
            Collections.sort(data);
    ...
    protected abstract List<Status> getStatuses(Twitter twitter, Paging paging)
    ...
    protected abstract boolean shouldFilterStatus(final SQLiteDatabase database,
