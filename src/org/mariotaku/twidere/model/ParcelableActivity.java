...
public class ParcelableActivity implements Comparable<ParcelableActivity>, JSONParcelable {
    ...
    public ParcelableActivity(final Activity activity, final long account_id,
        ...
        if (action == ACTION_FOLLOW || action == ACTION_MENTION
                || action == ACTION_LIST_MEMBER_ADDED) {
            ...
            target_users = null;
            for (int i = 0; i < targets_size; i++) {
                target_user_lists[i] = new ParcelableUserList(activity.getTargetUserLists()[i],
                        account_id,
                        large_profile_image);
            }
        } else {
            ...
            target_user_lists = null;
            for (int i = 0; i < targets_size; i++) {
                target_statuses[i] = new ParcelableStatus(activity.getTargetStatuses()[i],
                        account_id, false,
                        large_profile_image);
            }
        }
        ...
        if (action == ACTION_LIST_MEMBER_ADDED) {
            ...
            for (int i = 0; i < target_objects_size; i++) {
                target_object_user_lists[i] = new ParcelableUserList(
                        activity.getTargetObjectUserLists()[i],
            ...
            target_object_user_lists = null;
            for (int i = 0; i < target_objects_size; i++) {
                target_object_statuses[i] = new ParcelableStatus(
                        activity.getTargetObjectStatuses()[i], account_id,
    ...
    public ParcelableActivity(final JSONParcel in) {
        ...
        target_user_lists = in.readParcelableArray("target_user_lists",
                ParcelableUserList.JSON_CREATOR);
        target_object_user_lists = in.readParcelableArray("target_object_user_lists",
                ParcelableUserList.JSON_CREATOR);
        target_object_statuses = in.readParcelableArray("target_object_statuses",
                ParcelableStatus.JSON_CREATOR);
    }
    ...
    public boolean equals(final Object that) {
        if (!(that instanceof ParcelableActivity))
            return false;
