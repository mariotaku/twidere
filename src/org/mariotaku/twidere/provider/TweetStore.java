...
public final class TweetStore {
    ...
    public static final Uri CONTENT_URI_NULL = Uri.withAppendedPath(BASE_CONTENT_URI,
            CONTENT_PATH_NULL);

    public static final Uri[] STATUSES_URIS = new Uri[] {
            Statuses.CONTENT_URI, Mentions.CONTENT_URI,
            CachedStatuses.CONTENT_URI
    };
    public static final Uri[] CACHE_URIS = new Uri[] {
            CachedUsers.CONTENT_URI, CachedStatuses.CONTENT_URI,
    ...
    public static interface Accounts extends BaseColumns {
        ...
        public static final String[] COLUMNS = new String[] {
                _ID, NAME, SCREEN_NAME, ACCOUNT_ID, AUTH_TYPE,
                BASIC_AUTH_PASSWORD, OAUTH_TOKEN, TOKEN_SECRET, CONSUMER_KEY, CONSUMER_SECRET,
                REST_BASE_URL,
                SIGNING_REST_BASE_URL, OAUTH_BASE_URL, SIGNING_OAUTH_BASE_URL, PROFILE_IMAGE_URL,
                PROFILE_BANNER_URL,
                USER_COLOR, IS_ACTIVATED
        };

        public static final String[] TYPES = new String[] {
                TYPE_PRIMARY_KEY, TYPE_TEXT_NOT_NULL, TYPE_TEXT_NOT_NULL,
    ...
    public static interface CachedTrends extends CachedValues {
        ...
        public static final String[] TYPES = new String[] {
                TYPE_PRIMARY_KEY, TYPE_TEXT, TYPE_INT
        };
    ...
    public static interface CachedUsers extends CachedValues {
        ...
        public static final String[] COLUMNS = new String[] {
                _ID, USER_ID, CREATED_AT, NAME, SCREEN_NAME,
                DESCRIPTION_PLAIN, LOCATION, URL, PROFILE_IMAGE_URL, PROFILE_BANNER_URL,
                IS_PROTECTED, IS_VERIFIED,
                IS_FOLLOWING, FOLLOWERS_COUNT, FRIENDS_COUNT, STATUSES_COUNT, FAVORITES_COUNT,
                DESCRIPTION_HTML,
                DESCRIPTION_EXPANDED, URL_EXPANDED
        };

        public static final String[] TYPES = new String[] {
                TYPE_PRIMARY_KEY, TYPE_INT_UNIQUE, TYPE_INT, TYPE_TEXT,
                TYPE_TEXT, TYPE_TEXT, TYPE_TEXT, TYPE_TEXT, TYPE_TEXT, TYPE_TEXT, TYPE_BOOLEAN,
                TYPE_BOOLEAN,
                TYPE_BOOLEAN, TYPE_INT, TYPE_INT, TYPE_INT, TYPE_INT, TYPE_TEXT, TYPE_TEXT,
                TYPE_TEXT
        };
    ...
    public static interface DirectMessages extends BaseColumns {
        ...
        public static final String[] COLUMNS = new String[] {
                _ID, ACCOUNT_ID, MESSAGE_ID, MESSAGE_TIMESTAMP,
                SENDER_ID, RECIPIENT_ID, IS_OUTGOING, TEXT_HTML, TEXT_PLAIN, SENDER_NAME,
                RECIPIENT_NAME,
                SENDER_SCREEN_NAME, RECIPIENT_SCREEN_NAME, SENDER_PROFILE_IMAGE_URL,
                RECIPIENT_PROFILE_IMAGE_URL
        };
        public static final String[] TYPES = new String[] {
                TYPE_PRIMARY_KEY, TYPE_INT, TYPE_INT, TYPE_INT, TYPE_INT,
                TYPE_INT, TYPE_BOOLEAN, TYPE_TEXT, TYPE_TEXT, TYPE_TEXT, TYPE_TEXT, TYPE_TEXT,
                TYPE_TEXT, TYPE_TEXT,
                TYPE_TEXT
        };
        ...
        public static interface Conversation extends DirectMessages {
            ...
            public static final class QueryBuilder {
                ...
                public static final String buildByConversationId(final String[] projection,
                    ...
                    final Where account_id_where = new Where(String.format("%s = %d", ACCOUNT_ID,
                            account_id));
                    final Where sender_where = new Where(String.format("%s = %d", SENDER_ID,
                            conversation_id))
                            .and(account_id_where);
                    final Where recipient_where = new Where(String.format("%s = %d", RECIPIENT_ID,
                            conversation_id))
                            .and(account_id_where);
                    if (selection != null) {
                ...
                public static final String buildByScreenName(final String[] projection,
                        final long account_id,
                    ...
                    final Where account_id_where = new Where(String.format("%s = %d", ACCOUNT_ID,
                            account_id));
                    final Where sender_where = new Where(String.format("%s = '%s'", SENDER_ID,
                            screen_name))
                            .and(account_id_where);
                    final Where recipient_where = new Where(String.format("%s = '%s'",
                            RECIPIENT_ID, screen_name))
                            .and(account_id_where);
                    if (selection != null) {
                        qb.where(new Where(selection).and(sender_where));
                    } else {
        ...
        public static interface ConversationsEntry extends BaseColumns {
            ...
            public static class QueryBuilder {
                public static String build(final String where) {
                    final SQLQueryBuilder qb = new SQLQueryBuilder();
                    qb.select(new Columns(new Column(_ID), new Column(MESSAGE_TIMESTAMP),
                            new Column(MESSAGE_ID),
                            new Column(ACCOUNT_ID), new Column(IS_OUTGOING), new Column(NAME),
                            new Column(SCREEN_NAME),
                            new Column(PROFILE_IMAGE_URL), new Column(TEXT_HTML), new Column(
                                    CONVERSATION_ID)));
                    ...
                    entry_ids.select(new Columns(new Column(_ID), new Column(MESSAGE_TIMESTAMP),
                            new Column(MESSAGE_ID), new Column(ACCOUNT_ID), new Column("0",
                                    IS_OUTGOING), new Column(
                                    SENDER_NAME, NAME),
                            new Column(SENDER_SCREEN_NAME, SCREEN_NAME), new Column(
                                    SENDER_PROFILE_IMAGE_URL, PROFILE_IMAGE_URL), new Column(
                                    TEXT_HTML), new Column(
                                    SENDER_ID, CONVERSATION_ID)));
                    entry_ids.from(new Tables(Inbox.TABLE_NAME));
                    entry_ids.union();
                    entry_ids.select(new Columns(new Column(_ID), new Column(MESSAGE_TIMESTAMP),
                            new Column(MESSAGE_ID), new Column(ACCOUNT_ID), new Column("1",
                                    IS_OUTGOING), new Column(
                                    RECIPIENT_NAME, NAME), new Column(RECIPIENT_SCREEN_NAME,
                                    SCREEN_NAME), new Column(
                                    RECIPIENT_PROFILE_IMAGE_URL, PROFILE_IMAGE_URL), new Column(
                                    TEXT_HTML), new Column(
                    ...
                    final SQLQueryBuilder recent_inbox_msg_ids = new SQLQueryBuilder()
                            .select(new Column("MAX(" + MESSAGE_ID + ")"))
                            .from(new Tables(Inbox.TABLE_NAME))
                            .groupBy(new Column(SENDER_ID));
                    final SQLQueryBuilder recent_outbox_msg_ids = new SQLQueryBuilder()
                            .select(new Column("MAX(" + MESSAGE_ID + ")"))
                            .from(new Tables(Outbox.TABLE_NAME))
                            .groupBy(new Column(RECIPIENT_ID));
                    final SQLQueryBuilder message_conversation_ids = new SQLQueryBuilder();
                    message_conversation_ids.select(new Columns(new Column(MESSAGE_ID), new Column(
                            SENDER_ID,
                            CONVERSATION_ID)));
                    message_conversation_ids.from(new Tables(Inbox.TABLE_NAME));
                    message_conversation_ids.where(Where.in(new Column(MESSAGE_ID),
                            recent_inbox_msg_ids.build()));
        ...
        public static final class QueryBuilder {
            public static final String build(final String[] projection, final String selection,
    ...
    public static interface Drafts extends BaseColumns {
        ...
        public static final String[] COLUMNS = new String[] {
                _ID, TEXT, ACCOUNT_IDS, LOCATION, IMAGE_URI,
                IN_REPLY_TO_STATUS_ID, ATTACHED_IMAGE_TYPE, IS_POSSIBLY_SENSITIVE
        };

        public static final String[] TYPES = new String[] {
                TYPE_PRIMARY_KEY, TYPE_TEXT, TYPE_TEXT, TYPE_TEXT,
    ...
    public static interface Filters extends BaseColumns {
        ...
        public static final String[] TYPES = new String[] {
                TYPE_PRIMARY_KEY, TYPE_TEXT_NOT_NULL
        };
    ...
    public static interface Statuses extends BaseColumns {
        ...
        public static final String[] COLUMNS = new String[] {
                _ID, ACCOUNT_ID, STATUS_ID, USER_ID, STATUS_TIMESTAMP,
                TEXT_HTML, TEXT_PLAIN, TEXT_UNESCAPED, NAME, SCREEN_NAME, PROFILE_IMAGE_URL,
                IN_REPLY_TO_STATUS_ID,
                IN_REPLY_TO_SCREEN_NAME, SOURCE, LOCATION, RETWEET_COUNT, RETWEET_ID,
                RETWEETED_BY_ID,
                RETWEETED_BY_NAME, RETWEETED_BY_SCREEN_NAME, MY_RETWEET_ID, IS_RETWEET,
                IS_FAVORITE, IS_PROTECTED,
        ...
        public static final String[] TYPES = new String[] {
                ...
                TYPE_TEXT, TYPE_TEXT,
                TYPE_TEXT, TYPE_INT, TYPE_INT, TYPE_INT, TYPE_TEXT, TYPE_TEXT, TYPE_INT,
    ...
    public static interface Tabs extends BaseColumns {
        ...
        public static final String[] COLUMNS = new String[] {
                _ID, NAME, ICON, TYPE, ARGUMENTS, POSITION
        };

        public static final String[] TYPES = new String[] {
                TYPE_PRIMARY_KEY, TYPE_TEXT_NOT_NULL, TYPE_TEXT, TYPE_TEXT,
                TYPE_TEXT, TYPE_INT
        };
