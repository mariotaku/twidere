...
public final class TwidereDataProvider extends ContentProvider implements Constants {
    ...
    private final List<ParcelableStatus> mNewMentions = Collections
            .synchronizedList(new ArrayList<ParcelableStatus>());
    private final List<String> mNewMentionScreenNames = Collections
            .synchronizedList(new NoDuplicatesArrayList<String>());
    private final List<Long> mNewMentionAccounts = Collections
            .synchronizedList(new NoDuplicatesArrayList<Long>());
    ...
    private final List<String> mNewMessageScreenNames = Collections
            .synchronizedList(new NoDuplicatesArrayList<String>());
    private final List<Long> mNewMessageAccounts = Collections
            .synchronizedList(new NoDuplicatesArrayList<Long>());
            ...
            if (table != null && values != null) {
                ...
                for (final ContentValues contentValues : values) {
                    if (replace_on_conflict) {
                        mDatabase.insertWithOnConflict(table, null, contentValues,
                ...
                if (!"false".equals(uri.getQueryParameter(QUERY_PARAM_NOTIFY))) {
                    switch (table_id) {
                        case TABLE_ID_STATUSES: {
                            mNewStatusesCount += getAllStatusesCount(mContext, Statuses.CONTENT_URI)
                                    - old_count;
                            break;
                        }
                    }
    ...
    public Uri insert(final Uri uri, final ContentValues values) {
        try {
            ...
            final long row_id;
            if (replace_on_conflict) {
                row_id = mDatabase.insertWithOnConflict(table, null, values,
                        SQLiteDatabase.CONFLICT_REPLACE);
            } else {
        ...
        try {
            ...
            switch (table_id) {
                ...
                case TABLE_ID_DIRECT_MESSAGES_CONVERSATION: {
                    ...
                    if (segments.size() != 3)
                        return null;
                    final String query = Conversation.QueryBuilder.buildByConversationId(
                            projection,
                            Long.parseLong(segments.get(1)), Long.parseLong(segments.get(2)),
                            selection, sortOrder);
                ...
                case TABLE_ID_DIRECT_MESSAGES: {
                    final String query = DirectMessages.QueryBuilder.build(projection, selection,
                            sortOrder);
                    return mDatabase.rawQuery(query, selectionArgs);
                }
                case TABLE_ID_DIRECT_MESSAGES_CONVERSATIONS_ENTRY: {
                    return mDatabase.rawQuery(ConversationsEntry.QueryBuilder.build(selection),
                            null);
                }
            }
            if (table == null)
                return null;
            return mDatabase.query(table, projection, selection, selectionArgs, null, null,
                    sortOrder);
        } catch (final SQLException e) {
            throw new IllegalStateException(e);
        }
    ...
    private void buildNotification(final NotificationCompat.Builder builder, final String ticker,
            final String title,
            final String message, final int icon, final Bitmap large_icon,
        ...
        if (mNotificationIsAudible && !isNotificationsSilent(mContext)) {
            if (mPreferences.getBoolean(PREFERENCE_KEY_NOTIFICATION_HAVE_SOUND, false)) {
                ...
                final String path = mPreferences
                        .getString(PREFERENCE_KEY_NOTIFICATION_RINGTONE, "");
    ...
    private void checkReadPermission(final int id, final String table, final String[] projection) {
        switch (id) {
            ...
            case VIRTUAL_TABLE_ID_DNS: {
                if (!checkPermission(PERMISSION_PREFERENCES))
                    throw new SecurityException(
                            "Access preferences requires level PERMISSION_LEVEL_PREFERENCES");
                break;
            }
            case TABLE_ID_ACCOUNTS: {
                ...
                // permission level.
                if (projection == null
                        || ArrayUtils.contains(projection, Accounts.BASIC_AUTH_PASSWORD,
                                Accounts.OAUTH_TOKEN,
                                Accounts.TOKEN_SECRET) && !checkPermission(PERMISSION_ACCOUNTS))
                    throw new SecurityException("Access column "
                            + ArrayUtils.toString(projection, ',', true)
                ...
                if (!checkPermission(PERMISSION_READ))
                    throw new SecurityException("Access database " + table
                            + " requires level PERMISSION_LEVEL_READ");
                break;
            }
            case TABLE_ID_DIRECT_MESSAGES:
        ...
        switch (id) {
            ...
            case TABLE_ID_CACHED_HASHTAGS: {
                if (!checkPermission(PERMISSION_WRITE))
                    throw new SecurityException("Access database " + table
                            + " requires level PERMISSION_LEVEL_WRITE");
                break;
            }
        }
    }
    ...
    private void displayMentionsNotification(final Context context, final ContentValues[] values) {
        ...
        if (mNewMentions.size() == 1) {
            ...
            uri_builder.appendQueryParameter(QUERY_PARAM_ACCOUNT_ID,
                    String.valueOf(status.account_id));
            uri_builder.appendQueryParameter(QUERY_PARAM_STATUS_ID, String.valueOf(status.id));
            content_extras.putParcelable(INTENT_KEY_EXTRA_INTENT, new Intent(Intent.ACTION_VIEW,
                    uri_builder.build()));
        }
        content_intent.putExtras(content_extras);

        if (screen_names_size > 1) {
            title = mResources.getString(R.string.notification_mention_multiple,
                    display_screen_name ? "@"
        ...
        buildNotification(builder, title, title, status.text_plain, R.drawable.ic_stat_mention,
                null, content_intent,
                delete_intent);
        if (mNewMentions.isEmpty())
            return;
        if (mNewMentions.size() > 1) {
            ...
            for (int i = 0; i < max; i++) {
                ...
                style.addLine(Html.fromHtml("<b>"
                        + name
                        + "</b>: "
                        + stripMentionText(s.text_unescaped,
            ...
            if (accounts_count > 0) {
                ...
                final String[] names = display_screen_name ? getAccountScreenNames(context, ids,
                        true)
            ...
            builder.addAction(R.drawable.ic_menu_reply, context.getString(R.string.reply),
                    PendingIntent.getActivity(context, 0, reply_intent,
                            PendingIntent.FLAG_UPDATE_CURRENT));
            final NotificationCompat.BigTextStyle style = new NotificationCompat.BigTextStyle(
                    builder);
            style.bigText(stripMentionText(status.text_unescaped,
                    getAccountScreenName(context, status.account_id)));
            mNotificationManager.notify(NOTIFICATION_ID_MENTIONS, style.build());
        }
    }
    ...
    private void displayMessagesNotification(final Context context, final ContentValues[] values) {
        ...
        if (messages_size == 1) {
            ...
            uri_builder.appendQueryParameter(QUERY_PARAM_CONVERSATION_ID,
        ...
        if (screen_names_size > 1) {
            title = mResources.getString(R.string.notification_direct_message_multiple_users,
                    display_screen_name ? "@"
            ...
            title = mResources.getString(R.string.notification_direct_message,
                    display_screen_name ? "@"
                            + message.sender_screen_name : message.sender_name);
        }
        ...
        buildNotification(builder, title, title, text_plain, R.drawable.ic_stat_direct_message,
                null, content_intent,
                delete_intent);
        ...
        if (messages_size > 1) {
            ...
            for (int i = 0; i < max; i++) {
                ...
                if (s == null)
                    return;
                final String name = display_screen_name ? "@" + s.sender_screen_name
                        : s.sender_name;
    ...
    private Cursor getNotificationsCursor() {
        ...
        c.addRow(new Integer[] {
                NOTIFICATION_ID_HOME_TIMELINE, mNewStatusesCount
        });
        c.addRow(new Integer[] {
                NOTIFICATION_ID_MENTIONS, mNewMentions.size()
        });
        c.addRow(new Integer[] {
                NOTIFICATION_ID_DIRECT_MESSAGES, mNewMessages.size()
        });
        return c;
    }
    ...
    private Bitmap getProfileImageForNotification(final String profile_image_url) {
        ...
        final int w = mResources
                .getDimensionPixelSize(android.R.dimen.notification_large_icon_width);
        final int h = mResources
        ...
        final Bitmap profile_image = profile_image_file != null && profile_image_file.isFile() ? BitmapFactory
                .decodeFile(profile_image_file.getPath())
                : null;
        if (profile_image != null)
            return Bitmap.createScaledBitmap(profile_image, w, h, true);
        return Bitmap.createScaledBitmap(
                BitmapFactory.decodeResource(mResources, R.drawable.ic_profile_image_default),
                w, h, true);
    }
        ...
        switch (getTableId(uri)) {
            case TABLE_ID_STATUSES: {
                if (!mPreferences.getBoolean(PREFERENCE_KEY_NOTIFICATION_ENABLE_HOME_TIMELINE,
                    ...
                    return;
                final String message = mResources.getQuantityString(R.plurals.Ntweets,
                        mNewStatusesCount,
                        mNewStatusesCount);
                final Intent delete_intent = new Intent(BROADCAST_NOTIFICATION_CLEARED);
                final Bundle delete_extras = new Bundle();
                ...
                buildNotification(builder, mResources.getString(R.string.new_notifications),
                        message, message,
    ...
    private void preloadImages(final ContentValues... values) {
        if (values == null || !isOnWifi(mContext)
                && mPreferences.getBoolean(PREFERENCE_KEY_PRELOAD_WIFI_ONLY, true))
            return;
        for (final ContentValues v : values) {
            if (mPreferences.getBoolean(PREFERENCE_KEY_PRELOAD_PROFILE_IMAGES, false)) {
                ...
                final String sender_profile_image_url = v
                ...
                final String recipient_profile_image_url = v
    ...
    private static Cursor getPreferencesCursor(final SharedPreferences mPreferences,
            final String key) {
        ...
        for (final Map.Entry<String, ?> item : map.entrySet()) {
            ...
            c.addRow(new Object[] {
                    item.getKey(), ParseUtils.parseString(value), type
            });
        }
        return c;
    }
    ...
    private static String stripMentionText(final String text, final String my_screen_name) {
        if (text == null || my_screen_name == null)
            return text;
        final String temp = "@" + my_screen_name + " ";
        if (text.startsWith(temp))
            return text.substring(temp.length());
        return text;
    }
}
