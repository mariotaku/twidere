...
public class UpdateStatusService extends IntentService implements Constants {
    ...
    public void onCreate() {
        ...
        mUploader = mUseUploader ? ImageUploaderInterface.getInstance(app, uploader_component)
                : null;
        mShortener = mUseShortener ? TweetShortenerInterface.getInstance(app, shortener_component)
                : null;
    }
    ...
    public void showErrorMessage(final int action_res, final String message,
    ...
    protected List<SingleResponse<ParcelableStatus>> updateStatus(
            final ParcelableStatusUpdate pstatus) {
        ...
        if (pstatus.account_ids.length == 0)
            return Collections.emptyList();

        try {
            if (mUseUploader && mUploader == null)
                throw new ImageUploaderNotFoundException(this);
            if (mUseShortener && mShortener == null)
                throw new TweetShortenerNotFoundException(this);
            ...
            try {
                ...
                upload_result_uri = image_file != null && image_file.exists() && mUploader != null ? mUploader
                        .upload(
            ...
            final String unshortened_content = mUseUploader && upload_result_uri != null ? getImageUploadStatus(
                    this,
            ...
            try {
                ...
                shortened_content = should_shorten && mUseShortener ? mShortener.shorten(
                        unshortened_content,
            ...
            for (final long account_id : pstatus.account_ids) {
                final StatusUpdate status = new StatusUpdate(
                        should_shorten && mUseShortener ? shortened_content
                                : unshortened_content);
    ...
    private void handleUpdateStatusIntent(final Intent intent) {
        ...
        if (result.isEmpty()) {
            ...
            showErrorMessage(R.string.updating_status, getString(R.string.no_account_selected),
                    false);
        } else if (failed) {
    ...
    private void saveDrafts(final ParcelableStatusUpdate status, final List<Long> account_ids) {
        ...
        if (status.image_uri != null) {
            final int image_type = status.delete_image ? ATTACHED_IMAGE_TYPE_PHOTO
        ...
        final Notification notification = buildNotification(title, message,
                R.drawable.ic_stat_twitter, intent, null);
        mNotificationManager.notify(NOTIFICATION_ID_DRAFTS, notification);
    }

    private Notification updateUpdateStatusNotificaion(final int progress,
