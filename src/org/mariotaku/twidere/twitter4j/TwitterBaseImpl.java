...
import static org.mariotaku.twidere.twitter4j.http.HttpResponseCode.ENHANCE_YOUR_CLAIM;
import static org.mariotaku.twidere.twitter4j.http.HttpResponseCode.SERVICE_UNAVAILABLE;
...
import org.mariotaku.twidere.twitter4j.auth.Authorization;
import org.mariotaku.twidere.twitter4j.auth.AuthorizationFactory;
import org.mariotaku.twidere.twitter4j.auth.BasicAuthorization;
import org.mariotaku.twidere.twitter4j.auth.NullAuthorization;
import org.mariotaku.twidere.twitter4j.auth.OAuthAuthorization;
...
import org.mariotaku.twidere.twitter4j.http.HttpResponseEvent;
import org.mariotaku.twidere.twitter4j.http.HttpResponseListener;
import org.mariotaku.twidere.twitter4j.http.XAuthAuthorization;
import org.mariotaku.twidere.twitter4j.internal.json.InternalFactory;
import org.mariotaku.twidere.twitter4j.internal.json.InternalJSONImplFactory;
...
abstract class TwitterBaseImpl implements OAuthSupport, HttpResponseListener {
	...
	protected final void ensureAuthorizationEnabled() {
		if (!auth.isEnabled())
			throw new IllegalStateException(
					"Authentication credentials are missing. See http://org.mariotaku.twidere.twitter4j.org/configuration.html for the detail.");
	}

	protected final void ensureOAuthEnabled() {
		if (!(auth instanceof OAuthAuthorization))
			throw new IllegalStateException(
					"OAuth required. Authentication credentials are missing. See http://org.mariotaku.twidere.twitter4j.org/configuration.html for the detail.");
	}
