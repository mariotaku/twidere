...
package org.mariotaku.twidere.twitter4j.conf;

import org.mariotaku.twidere.twitter4j.http.HostAddressResolver;
import org.mariotaku.twidere.twitter4j.http.HttpClient;

/**
 * A builder that can be used to construct a org.mariotaku.twidere.twitter4j
 * configuration with desired settings. This builder has sensible defaults such
 * that {@code new ConfigurationBuilder().build()} would create a usable
 * configuration. This configuration builder is useful for clients that wish to
 * configure org.mariotaku.twidere.twitter4j in unit tests or from command line
 * flags for example.
 * 
