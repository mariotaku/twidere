...
public final class DatabaseUpgradeHelper {
    ...
    public static void safeUpgrade(final SQLiteDatabase db, final String table,
            final String[] new_cols,
        ...
        if (!drop_directly) {
            ...
            while (!cur.isAfterLast()) {
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    final String col_alias = column_alias != null
                            && column_alias.containsKey(new_col)
                            && ArrayUtils.contains(old_cols, column_alias.get(new_col)) ? column_alias
                            .get(new_col)
                            : new_col;
    ...
    private static String[] getBatchTypeString(final SQLiteDatabase db, final String table,
    ...
    private static int getTypeInt(final String type) {
        ...
        else if (TYPE_BLOB.equalsIgnoreCase(type_main))
            return FIELD_TYPE_BLOB;
        throw new IllegalStateException("Unknown field type " + type);
    }

    private static String getTypeString(final SQLiteDatabase db, final String table,
            final String column) {
    ...
    private static boolean isTypeCompatible(final String old_type, final String new_type,
        ...
        if (old_type != null && new_type != null) {
            final int old_idx = old_type.contains("(") ? old_type.indexOf("(") : old_type
                    .indexOf(" ");
            final int new_idx = new_type.contains("(") ? new_type.indexOf("(") : new_type
                    .indexOf(" ");
            ...
            if (treat_null_as_compatible)
                return "NULL".equalsIgnoreCase(old_type_main)
                        || "NULL".equalsIgnoreCase(new_type_main)
    ...
    private static boolean shouldUpgrade(final String[] old_cols, final String[] old_types,
            final String[] new_cols,
        ...
        if (old_cols.length != new_cols.length)
            return true;
        if (!ArrayUtils.contentMatch(old_cols, new_cols))
            return true;
        ...
        for (final String col_name : old_keyset) {
            if (!isTypeCompatible(old_map.get(col_name), new_map.get(col_name), true))
                return true;
        }
        return false;
    }
