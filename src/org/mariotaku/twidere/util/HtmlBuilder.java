...
public class HtmlBuilder {
    ...
    public HtmlBuilder(final String source, final boolean strict, final boolean is_escaped,
            final boolean re_escape) {
        if (source == null)
    ...
    public boolean addLink(final String link, final String display, final int start, final int end,
        ...
        if (start < 0 || end < 0 || start > end || end > string_length) {
            final String message = String.format("text:%s, length:%d, start:%d, end:%d", orig,
                    string_length, start,
                    end);
            if (throw_exceptions)
        ...
        if (hasLink(start, end)) {
            final String message = String
                    .format(
                            "link already added in this range! text:%s, link:%s, display:%s, start:%d, end:%d",
                            orig, link,
                            display, start, end);
            if (throw_exceptions)
    ...
    public String build() {
        if (links.size() == 0)
            return escapeSource(ArrayUtils.mergeArrayToString(array));
        ...
        for (int i = 0; i < links_size; i++) {
            ...
            if (i == 0) {
                if (start >= 0 && start <= string_length) {
                    builder.append(escapeSource(ArrayUtils.mergeArrayToString(ArrayUtils.subArray(
                            array, 0, start))));
                }
            } else if (i > 0) {
            ...
            if (start >= 0 && start <= end && end <= string_length) {
                builder.append(!isEmpty(spec.display) ? spec.display_is_html ? spec.display
                        : toHtml(spec.display)
                        : spec.link);
            }
            builder.append("</a>");
    ...
    public String toString() {
        return "HtmlBuilder{array=" + Arrays.toString(array) + ", string_length=" + string_length
                + ", strict="
    ...
    static final class LinkSpec implements Comparable<LinkSpec> {
        ...
        public boolean equals(final Object obj) {
            ...
            if (display == null) {
                ...
                return false;
            if (display_is_html != other.display_is_html)
                return false;
            if (end != other.end)
                return false;
            if (link == null) {
        ...
        @Override
        public String toString() {
            return "LinkSpec{link=" + link + ", display=" + display + ", start=" + start + ", end="
                    + end
