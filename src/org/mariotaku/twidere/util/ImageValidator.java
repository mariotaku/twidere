...
public class ImageValidator {
    ...
    private static final byte[] PNG_TAIL = {
            0x49, 0x45, 0x4E, 0x44, (byte) 0xAE, 0x42, 0x60, (byte) 0x82
    };

    private static final byte[] JPEG_HEAD = {
            0xFFFFFFFF, 0xFFFFFFD8
    };
    private static final byte[] JPEG_TAIL = {
            0xFFFFFFFF, 0xFFFFFFD9
    };
    ...
    private static boolean checkHeadTailValidity(final String file, final byte[] head,
            final byte[] tail) {
        if (file == null)
            return false;
        ...
        try {
            ...
            raf.seek(0);
            if (raf.read(buffer) != buffer.length || !Arrays.equals(buffer, head))
                return false;
            // Read tail.
