...
public class OAuthPasswordAuthenticator implements Constants {
    ...
    private InputStream getHTTPContent(final String url_string, final boolean post,
            final HttpParameter[] params)
    ...
    private synchronized void readAuthenticityToken(final InputStream stream) throws SAXException,
            IOException {
    ...
    private synchronized void readOAuthPIN(final InputStream stream) throws SAXException,
            IOException {
        final InputSource source = new InputSource(stream);
        final Parser parser = new Parser();
        parser.setProperty(Parser.schemaProperty, HtmlParser.schema);
