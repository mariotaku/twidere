...
public class SaveImageTask extends AsyncTask<Void, Void, File> implements Constants {
    ...
    protected void onPostExecute(final File result) {
        ...
        if (result != null && result.exists()) {
            Crouton.showText(activity,
                    activity.getString(R.string.file_saved_to, result.getPath()),
    ...
    public static File saveImage(final Context context, final File image_file) {
        if (context == null && image_file == null)
            return null;
        try {
            ...
            final File pub_dir = Environment
                    .getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES);
            if (pub_dir == null)
                return null;
            final File save_dir = new File(pub_dir, "Twidere");
            if (!save_dir.isDirectory() && !save_dir.mkdirs())
                return null;
