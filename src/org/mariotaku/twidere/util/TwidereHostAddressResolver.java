...
public class TwidereHostAddressResolver implements Constants, HostAddressResolver {
    ...
    public TwidereHostAddressResolver(final Context context, final boolean local_only) {
        mHostMapping = context.getSharedPreferences(HOST_MAPPING_PREFERENCES_NAME,
                Context.MODE_PRIVATE);
        mPreferences = context.getSharedPreferences(SHARED_PREFERENCES_NAME, Context.MODE_PRIVATE);
        final String address = mPreferences.getString(PREFERENCE_KEY_DNS_SERVER,
    ...
    public String resolve(final String host) throws IOException {
        if (host == null || !mPreferences.getBoolean(PREFERENCE_KEY_IGNORE_SSL_ERROR, false))
            return null;
        // First, I'll try to load address cached.
        if (mHostCache.containsKey(host)) {
            if (mIsDebugBuild) {
                Log.d(RESOLVER_LOGTAG, "Got cached address " + mHostCache.get(host) + " for host "
                        + host);
            }
        ...
        if (host_segments_length > 2) {
            ...
            if (mHostMapping.contains(top_domain)) {
                ...
                if (mIsDebugBuild) {
                    Log.d(RESOLVER_LOGTAG, "Got mapped address (top domain) " + host_addr
                            + " for host " + host);
                }
                return host_addr;
            }
        ...
        if (mDns != null && mPreferences.getBoolean(PREFERENCE_KEY_TCP_DNS_QUERY, false)) {
            ...
            for (final Record record : records) {
                ...
                if (mHostCache.put(host, host_addr) != null) {
                    if (mIsDebugBuild) {
                        Log.d(RESOLVER_LOGTAG, "Resolved address " + host_addr + " for host "
                                + host);
                    }
                    return host_addr;
                }
