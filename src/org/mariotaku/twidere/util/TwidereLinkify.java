...
public class TwidereLinkify {
    ...
    public static final int[] ALL_LINK_TYPES = new int[] {
            LINK_TYPE_LINK, LINK_TYPE_MENTION, LINK_TYPE_HASHTAG,
            LINK_TYPE_STATUS, LINK_TYPE_LINK_WITH_IMAGE_EXTENSION, LINK_TYPE_ALL_AVAILABLE_IMAGE,
            LINK_TYPE_CASHTAG
    };
    ...
    public static final Pattern PATTERN_TWITTER_PROFILE_IMAGES = Pattern.compile(
    ...
    public static final Pattern PATTERN_TWITTER_STATUS = Pattern.compile(
            STRING_PATTERN_TWITTER_STATUS,
            Pattern.CASE_INSENSITIVE);
    ...
    public final void applyAllLinks(final TextView view, final long account_id,
    ...
    public final void applyUserProfileLink(final TextView view, final long account_id,
            final long user_id,
        ...
        if (user_id > 0) {
            applyLink(String.valueOf(user_id), 0, string.length(), string, account_id,
                    LINK_TYPE_USER_ID, false);
     ...
     *            link text
     */
    private final void addLinks(final SpannableString string, final long account_id,
            final int type,
            final boolean sensitive) {
        switch (type) {
            ...
            case LINK_TYPE_LINK_WITH_IMAGE_EXTENSION: {
                ...
                for (final URLSpan span : spans) {
                    ...
                    if (PATTERN_IMAGES.matcher(url).matches()) {
                        string.removeSpan(span);
                        applyLink(url, start, end, string, account_id,
            ...
            case LINK_TYPE_LINK: {
                ...
                for (final URLSpan span : spans) {
                    ...
                    string.removeSpan(span);
                    applyLink(span.getURL(), start, end, string, account_id, LINK_TYPE_LINK,
                            sensitive);
                }
                for (final Extractor.Entity entity : mExtractor.extractURLsWithIndices(ParseUtils
                        .parseString(string))) {
                    ...
                    applyLink(entity.getValue(), start, end, string, account_id, LINK_TYPE_LINK,
                            sensitive);
                }
                break;
            }
                ...
                for (final URLSpan span : spans) {
                    ...
                    if (matcher.matches()) {
                        final PreviewImage spec = PreviewImage
                                .getAllAvailableImage(matcher.group());
                        final int start = string.getSpanStart(span);
                        final int end = string.getSpanEnd(span);
    ...
    private final boolean addMentionOrListLinks(final Spannable spannable, final long account_id) {
        ...
        while (matcher.find()) {
            ...
            if (list_start >= 0 && list_end >= 0) {
                applyLink(mention + "/" + list, list_start, list_end, spannable, account_id,
                        LINK_TYPE_LIST, false);
            }
            hasMatches = true;
        }
        ...
        for (final URLSpan span : spans) {
            ...
            if (m.matches()) {
                ...
                applyLink(screen_name + "/" + list_name, start, end, spannable, account_id,
                        LINK_TYPE_LIST, false);
            }
        }
        return hasMatches;
    ...
    private final void applyLink(final String url, final int start, final int end,
    ...
    private final void applyLink(final String url, final String orig, final int start,
            final int end,
            final Spannable text, final long account_id, final int type, final boolean sensitive) {
        final LinkSpan span = new LinkSpan(url, orig, account_id, type, sensitive,
    ...
    static class LinkSpan extends URLSpan {
        ...
        public LinkSpan(final String url, final long account_id, final int type,
                final boolean sensitive,
        ...
        public LinkSpan(final String url, final String orig, final long account_id, final int type,
                final boolean sensitive, final OnLinkClickListener listener,
                final int highlight_style) {
            super(url);
            this.url = url;
            this.orig = orig;
