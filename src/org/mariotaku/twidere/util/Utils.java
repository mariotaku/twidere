...
public final class Utils implements Constants {
    ...
    public static final Pattern PATTERN_XML_RESOURCE_IDENTIFIER = Pattern
            .compile("res\\/xml\\/([\\w_]+)\\.xml");
    public static final Pattern PATTERN_RESOURCE_IDENTIFIER = Pattern
            .compile("@([\\w_]+)\\/([\\w_]+)");

    private static final UriMatcher CONTENT_PROVIDER_URI_MATCHER = new UriMatcher(
            UriMatcher.NO_MATCH);
    private static final UriMatcher LINK_HANDLER_URI_MATCHER = new UriMatcher(UriMatcher.NO_MATCH);
    static {
        CONTENT_PROVIDER_URI_MATCHER
                .addURI(TweetStore.AUTHORITY, TABLE_STATUSES, TABLE_ID_STATUSES);
        CONTENT_PROVIDER_URI_MATCHER
                .addURI(TweetStore.AUTHORITY, TABLE_ACCOUNTS, TABLE_ID_ACCOUNTS);
        CONTENT_PROVIDER_URI_MATCHER
                .addURI(TweetStore.AUTHORITY, TABLE_MENTIONS, TABLE_ID_MENTIONS);
        CONTENT_PROVIDER_URI_MATCHER.addURI(TweetStore.AUTHORITY, TABLE_DRAFTS, TABLE_ID_DRAFTS);
        CONTENT_PROVIDER_URI_MATCHER.addURI(TweetStore.AUTHORITY, TABLE_CACHED_USERS,
                TABLE_ID_CACHED_USERS);
        CONTENT_PROVIDER_URI_MATCHER.addURI(TweetStore.AUTHORITY, TABLE_FILTERED_USERS,
                TABLE_ID_FILTERED_USERS);
        CONTENT_PROVIDER_URI_MATCHER.addURI(TweetStore.AUTHORITY, TABLE_FILTERED_KEYWORDS,
                TABLE_ID_FILTERED_KEYWORDS);
        CONTENT_PROVIDER_URI_MATCHER.addURI(TweetStore.AUTHORITY, TABLE_FILTERED_SOURCES,
                TABLE_ID_FILTERED_SOURCES);
        CONTENT_PROVIDER_URI_MATCHER.addURI(TweetStore.AUTHORITY, TABLE_FILTERED_LINKS,
                TABLE_ID_FILTERED_LINKS);
        CONTENT_PROVIDER_URI_MATCHER.addURI(TweetStore.AUTHORITY, TABLE_DIRECT_MESSAGES,
        ...
        CONTENT_PROVIDER_URI_MATCHER.addURI(TweetStore.AUTHORITY,
                TABLE_DIRECT_MESSAGES_CONVERSATION + "/#/#",
                TABLE_ID_DIRECT_MESSAGES_CONVERSATION);
        CONTENT_PROVIDER_URI_MATCHER.addURI(TweetStore.AUTHORITY,
                TABLE_DIRECT_MESSAGES_CONVERSATION_SCREEN_NAME
                        + "/#/*", TABLE_ID_DIRECT_MESSAGES_CONVERSATION_SCREEN_NAME);
        CONTENT_PROVIDER_URI_MATCHER.addURI(TweetStore.AUTHORITY,
                TABLE_DIRECT_MESSAGES_CONVERSATIONS_ENTRY,
                TABLE_ID_DIRECT_MESSAGES_CONVERSATIONS_ENTRY);
        CONTENT_PROVIDER_URI_MATCHER.addURI(TweetStore.AUTHORITY, TABLE_TRENDS_LOCAL,
                TABLE_ID_TRENDS_LOCAL);
        CONTENT_PROVIDER_URI_MATCHER.addURI(TweetStore.AUTHORITY, TABLE_TABS, TABLE_ID_TABS);
        CONTENT_PROVIDER_URI_MATCHER.addURI(TweetStore.AUTHORITY, TABLE_CACHED_STATUSES,
                TABLE_ID_CACHED_STATUSES);
        CONTENT_PROVIDER_URI_MATCHER.addURI(TweetStore.AUTHORITY, TABLE_CACHED_HASHTAGS,
        ...
        CONTENT_PROVIDER_URI_MATCHER.addURI(TweetStore.AUTHORITY, TABLE_NOTIFICATIONS,
                VIRTUAL_TABLE_ID_NOTIFICATIONS);
        CONTENT_PROVIDER_URI_MATCHER.addURI(TweetStore.AUTHORITY, TABLE_PERMISSIONS,
                VIRTUAL_TABLE_ID_PERMISSIONS);
        CONTENT_PROVIDER_URI_MATCHER.addURI(TweetStore.AUTHORITY, TABLE_DNS + "/*",
                VIRTUAL_TABLE_ID_DNS);
        CONTENT_PROVIDER_URI_MATCHER.addURI(TweetStore.AUTHORITY, TABLE_CACHED_IMAGES,
        ...
        LINK_HANDLER_URI_MATCHER.addURI(AUTHORITY_INCOMING_FRIENDSHIPS, null,
        ...
        LINK_HANDLER_URI_MATCHER.addURI(AUTHORITY_STATUS_RETWEETERS, null,
    ...
    private static Map<Long, Integer> sUserColors = new LinkedHashMap<Long, Integer>(512, 0.75f,
            true);

    private static Map<Long, String> sUserNicknames = new LinkedHashMap<Long, String>(512, 0.75f,
            true);
    ...
    public static void addIntentToMenu(final Context context, final Menu menu,
            final Intent query_intent) {
        ...
        for (final ResolveInfo info : activities) {
            ...
            if (icon instanceof BitmapDrawable) {
                ...
                final Bitmap bitmap = Bitmap.createBitmap(orig.getWidth() + paddings * 2,
                        orig.getHeight() + paddings
    ...
    public static String buildActivatedStatsWhereClause(final Context context,
    ...
    public static Uri buildDirectMessageConversationUri(final long account_id,
            final long conversation_id,
            final String screen_name) {
        if (conversation_id <= 0 && screen_name == null)
            return TweetStore.CONTENT_URI_NULL;
        final Uri.Builder builder = conversation_id > 0 ? DirectMessages.Conversation.CONTENT_URI
                .buildUpon()
    ...
    public static String buildStatusFilterWhereClause(final String table, final String selection,
        ...
        // TODO
        if (enable_in_rts) {
            builder.append(" OR " + table + "." + Statuses.RETWEETED_BY_SCREEN_NAME
                    + " IN ( SELECT "
                    + TABLE_FILTERED_USERS + "." + Filters.Users.VALUE + " FROM "
                    + TABLE_FILTERED_USERS + " )");
        }
        ...
        builder.append(" WHERE " + table + "." + Statuses.TEXT_PLAIN + " LIKE '%'||"
                + TABLE_FILTERED_KEYWORDS + "."
    ...
    public static synchronized void cleanDatabasesByItemLimit(final Context context) {
        ...
        final int item_limit = context.getSharedPreferences(SHARED_PREFERENCES_NAME,
        ...
        for (final long account_id : getAccountIds(context)) {
            ...
            for (final Uri uri : STATUSES_URIS) {
                ...
                qb.limit(item_limit);
                final Where where = Where.notIn(new Column(Statuses._ID), qb.build()).and(
                        account_where);
                resolver.delete(uri, where.getSQL(), null);
            }
            for (final Uri uri : DIRECT_MESSAGES_URIS) {
                final String table = getTableNameByUri(uri);
                final Where account_where = new Where(DirectMessages.ACCOUNT_ID + " = "
                        + account_id);
                final SQLQueryBuilder qb = new SQLQueryBuilder();
                ...
                qb.limit(item_limit);
                final Where where = Where.notIn(new Column(DirectMessages._ID), qb.build()).and(
                        account_where);
                resolver.delete(uri, where.getSQL(), null);
            }
        }
    ...
    public static void clearListViewChoices(final ListView view) {
        ...
        final int position = view.getFirstVisiblePosition(), offset = Utils
                .getFirstChildOffset(view);
    ...
    public static void configBaseAdapter(final Context context, final IBaseAdapter adapter) {
        if (context == null)
            return;
        final SharedPreferences pref = context.getSharedPreferences(SHARED_PREFERENCES_NAME,
                Context.MODE_PRIVATE);
        adapter.setDisplayProfileImage(pref.getBoolean(PREFERENCE_KEY_DISPLAY_PROFILE_IMAGE, true));
        adapter.setNameDisplayOption(pref.getString(PREFERENCE_KEY_NAME_DISPLAY_OPTION,
    ...
    public static Fragment createFragmentForIntent(final Context context, final Intent intent) {
        ...
        switch (matchLinkId(uri)) {
            ...
            case LINK_ID_DIRECT_MESSAGES_CONVERSATION: {
                ...
                final String param_conversation_id = uri
                        .getQueryParameter(QUERY_PARAM_CONVERSATION_ID);
                final String param_screen_name = uri.getQueryParameter(QUERY_PARAM_SCREEN_NAME);
            ...
            case LINK_ID_LIST_DETAILS: {
                ...
                if (isEmpty(param_list_id)
                        && (isEmpty(param_list_name) || isEmpty(param_screen_name)
                                && isEmpty(param_user_id)))
                    return null;
    ...
    public static Intent createPickImageIntent(final Uri uri, final Integer outputX,
    ...
    public static Intent createTakePhotoIntent(final Uri uri, final Integer outputX,
            final Integer outputY,
            final Integer aspectX, final Integer aspectY, final boolean scaleUpIfNeeded) {
    ...
    public static ParcelableStatus findStatus(final Context context, final long account_id,
        ...
        if (status == null || status.getId() <= 0)
            return null;
        final String where = Statuses.ACCOUNT_ID + " = " + account_id + " AND "
                + Statuses.STATUS_ID + " = "
                + status.getId();
        final ContentResolver resolver = context.getContentResolver();
        final boolean large_profile_image = context.getResources().getBoolean(
                R.bool.hires_profile_image);
    ...
    public static ParcelableStatus findStatusInDatabases(final Context context,
            final long account_id,
            final long status_id) {
        if (context == null)
            return null;
    ...
    public static String formatSameDayTime(final Context context, final long timestamp) {
        ...
        if (DateUtils.isToday(timestamp))
            return DateUtils.formatDateTime(context, timestamp,
                    DateFormat.is24HourFormat(context) ? DateUtils.FORMAT_SHOW_TIME
    ...
    public static String formatUserDescription(final User user) {
        ...
        if (urls != null) {
            for (final URLEntity url : urls) {
                ...
                if (expanded_url != null) {
                    builder.addLink(ParseUtils.parseString(expanded_url), url.getDisplayURL(),
                            url.getStart(),
                            url.getEnd());
                }
            }
    ...
    public static int[] getAccountColors(final Context context, final long[] account_ids) {
        if (context == null || account_ids == null)
            return null;
    ...
    public static long getAccountId(final Context context, final String screen_name) {
        if (context == null || isEmpty(screen_name))
            return -1;
        long user_id = -1;

        final Cursor cur = context.getContentResolver()
                .query(Accounts.CONTENT_URI, new String[] {
                    Accounts.ACCOUNT_ID
    ...
    public static int getAllStatusesCount(final Context context, final Uri uri) {
        ...
        final Cursor cur = resolver.query(
                ...
                buildStatusFilterWhereClause(getTableNameByUri(uri), null,
                        shouldEnableFiltersForRTs(context)), null,
                null);
        if (cur == null)
            return 0;
        try {
    ...
    public static boolean getAsBoolean(final ContentValues values, final String key,
            final boolean def) {
    ...
    public static Bitmap getColorPreviewBitmap(final Context context, final int color) {
        ...
        final float[] points = new float[] {
                0, 0, width, 0, 0, 0, 0, height, width, 0, width, height, 0, height,
                width, height
        };
    ...
    public static Twitter getDefaultTwitterInstance(final Context context,
    ...
    public static Twitter getDefaultTwitterInstance(final Context context,
        ...
        if (context == null)
            return null;
        return getTwitterInstance(context, getDefaultAccountId(context), include_entities,
                include_retweets,
                use_httpclient);
    }
    ...
    public static List<SupportTabSpec> getHomeTabs(final Context context) {
        ...
        final ContentResolver resolver = context.getContentResolver();
        final Cursor cur = resolver.query(Tabs.CONTENT_URI, Tabs.COLUMNS, null, null,
                Tabs.DEFAULT_SORT_ORDER);
        if (cur != null) {
            cur.moveToFirst();
            final int idx_name = cur.getColumnIndex(Tabs.NAME), idx_icon = cur
                    .getColumnIndex(Tabs.ICON), idx_type = cur
            ...
            while (!cur.isAfterLast()) {
                ...
                final Class<? extends Fragment> fragment = CustomTabConfiguration.get(type)
                        .getFragmentClass();
                if (name != null && fragment != null) {
                    tabs.add(new SupportTabSpec(name, CustomTabConfiguration
    ...
    public static HttpClientWrapper getHttpClient(final int timeout_millis,
            final boolean ignore_ssl_error,
    ...
    public static HttpClientWrapper getImageLoaderHttpClient(final Context context) {
        ...
        final HostAddressResolver resolver = TwidereApplication.getInstance(context)
                .getHostAddressResolver();
    ...
    public static String getImagePathFromUri(final Context context, final Uri uri) {
        ...
        final String media_uri_start = ParseUtils
                .parseString(MediaStore.Images.Media.EXTERNAL_CONTENT_URI);
    ...
    public static String getImageUploadStatus(final Context context, final CharSequence link,
            final CharSequence text) {
        if (context == null)
            return null;
        String image_upload_format = context.getSharedPreferences(SHARED_PREFERENCES_NAME,
                Context.MODE_PRIVATE)
                .getString(PREFERENCE_KEY_IMAGE_UPLOAD_FORMAT,
                        PREFERENCE_DEFAULT_IMAGE_UPLOAD_FORMAT);
        ...
        return image_upload_format.replace(FORMAT_PATTERN_LINK, link).replace(FORMAT_PATTERN_TEXT,
                text);
    }
    ...
    public static String getMapStaticImageUri(final double lat, final double lng, final int zoom,
            final int w,
            final int h, final Locale locale) {
        return String.format(MAPS_STATIC_IMAGE_URI_TEMPLATE, zoom, w, h, locale.toString(), lat,
                lng, lat, lng);
    }
    ...
    public static String getMapStaticImageUri(final double lat, final double lng, final View v) {
        ...
        return getMapStaticImageUri(lat, lng, 12, v.getMeasuredWidth(), v.getMeasuredHeight(), v
                .getResources()
    ...
    public static int getNameDisplayOptionInt(final String option) {
        if (NAME_DISPLAY_OPTION_NAME.equals(option))
            ...
            return NAME_DISPLAY_OPTION_CODE_SCREEN_NAME;
    ...
    public static long[] getNewestMessageIdsFromDatabase(final Context context, final Uri uri,
        ...
        for (final long account_id : account_ids) {
            ...
            final Cursor cur = resolver.query(uri, cols, where, null,
    ...
    public static String getNonEmptyString(final SharedPreferences pref, final String key,
            final String def) {
        if (pref == null)
            return def;
    ...
    public static Proxy getProxy(final Context context) {
        ...
        final int proxy_port = ParseUtils
                .parseInt(prefs.getString(PREFERENCE_KEY_PROXY_PORT, "-1"));
    ...
    public static String getQuoteStatus(final Context context, final String screen_name,
            final String text) {
        if (context == null)
            return null;
        String quote_format = context.getSharedPreferences(SHARED_PREFERENCES_NAME,
    ...
    public static HttpResponse getRedirectedHttpResponse(final HttpClientWrapper client,
            final String url)
            throws TwitterException {
        if (url == null)
            return null;
    ...
    public static String getShareStatus(final Context context, final CharSequence title,
            final CharSequence text) {
        if (context == null)
            return null;
        String share_format = context.getSharedPreferences(SHARED_PREFERENCES_NAME,
                Context.MODE_PRIVATE).getString(
    ...
    public static int getStatusBackground(final boolean is_mention, final boolean is_favorite,
    ...
    public static Twitter getTwitterInstance(final Context context, final long account_id,
            final boolean include_entities, final boolean include_retweets,
            final boolean use_apache_httpclient) {
        ...
        final boolean enable_gzip_compressing = prefs.getBoolean(PREFERENCE_KEY_GZIP_COMPRESSING,
                true);
        ...
        // versions
        final String pref_consumer_key = prefs.getString(PREFERENCE_KEY_CONSUMER_KEY,
                TWITTER_CONSUMER_KEY);
        final String pref_consumer_secret = prefs.getString(PREFERENCE_KEY_CONSUMER_SECRET,
                TWITTER_CONSUMER_SECRET);
        final StringBuilder where = new StringBuilder();
        where.append(Accounts.ACCOUNT_ID + " = " + account_id);
        final Cursor c = context.getContentResolver().query(Accounts.CONTENT_URI, Accounts.COLUMNS,
                where.toString(),
                null, null);
        if (c == null)
            return null;
        try {
            if (c.getCount() > 0) {
                ...
                if (enable_proxy) {
                    final String proxy_host = prefs.getString(PREFERENCE_KEY_PROXY_HOST, null);
                    final int proxy_port = ParseUtils.parseInt(prefs.getString(
                ...
                final String signing_rest_base_url = c.getString(c
                        .getColumnIndex(Accounts.SIGNING_REST_BASE_URL));
                final String oauth_base_url = c
                        .getString(c.getColumnIndex(Accounts.OAUTH_BASE_URL));
                final String signing_oauth_base_url = c.getString(c
                        .getColumnIndex(Accounts.SIGNING_OAUTH_BASE_URL));
                final String consumer_key = trim(c.getString(c
                        .getColumnIndex(Accounts.CONSUMER_KEY)));
                final String consumer_secret = trim(c.getString(c
                        .getColumnIndex(Accounts.CONSUMER_SECRET)));
                ...
                switch (c.getInt(c.getColumnIndexOrThrow(Accounts.AUTH_TYPE))) {
                    ...
                    case Accounts.AUTH_TYPE_XAUTH: {
                        ...
                        final String oauth_token = c.getString(c
                                .getColumnIndexOrThrow(Accounts.OAUTH_TOKEN));
                        final String token_secret = c.getString(c
                                .getColumnIndexOrThrow(Accounts.TOKEN_SECRET));
                        if (isEmpty(oauth_token) || isEmpty(token_secret))
                            return null;
                        return new TwitterFactory(cb.build()).getInstance(new AccessToken(
                                oauth_token, token_secret));
                    }
                    case Accounts.AUTH_TYPE_BASIC: {
                        final String screen_name = c.getString(c
                                .getColumnIndexOrThrow(Accounts.SCREEN_NAME));
    ...
    public static int getUserColor(final Context context, final long user_id,
            final boolean ignore_cache) {
        if (context == null || user_id == -1)
            return Color.TRANSPARENT;
        if (!ignore_cache && sUserColors.containsKey(user_id))
            return sUserColors.get(user_id);
        final SharedPreferences prefs = context.getSharedPreferences(USER_COLOR_PREFERENCES_NAME,
                Context.MODE_PRIVATE);
    ...
    public static String getUserNickname(final Context context, final long user_id,
        ...
        if (context == null || user_id == -1)
            return null;
        if (!ignore_cache && sUserNicknames.containsKey(user_id))
            return sUserNicknames.get(user_id);
        final SharedPreferences prefs = context.getSharedPreferences(
                USER_NICKNAME_PREFERENCES_NAME,
                Context.MODE_PRIVATE);
    ...
    public static void initAccountColor(final Context context) {
        ...
        final Cursor cur = context.getContentResolver().query(Accounts.CONTENT_URI,
                new String[] {
                        Accounts.ACCOUNT_ID, Accounts.USER_COLOR
                }, null, null, null);
        if (cur == null)
            return;
        final int id_idx = cur.getColumnIndex(Accounts.ACCOUNT_ID), color_idx = cur
                .getColumnIndex(Accounts.USER_COLOR);
        cur.moveToFirst();
        while (!cur.isAfterLast()) {
    ...
    public static boolean isFiltered(final SQLiteDatabase database, final ParcelableStatus status) {
        if (status == null)
            return false;
        return isFiltered(database, status.text_plain, status.text_html, status.user_screen_name,
                status.source);
    }

    public static boolean isFiltered(final SQLiteDatabase database, final String text_plain,
            final String text_html,
            final String screen_name, final String source) {
        if (database == null)
            return false;
        if (text_plain == null && text_html == null && screen_name == null && source == null)
            return false;
        final StringBuilder builder = new StringBuilder();
        ...
        if (text_plain != null) {
            ...
            builder.append("(SELECT 1 IN (SELECT ? LIKE '%'||" + TABLE_FILTERED_KEYWORDS + "."
                    + Filters.VALUE
        ...
        if (text_html != null) {
            ...
            builder.append("(SELECT 1 IN (SELECT ? LIKE '%<a href=\"%'||" + TABLE_FILTERED_LINKS
                    + "." + Filters.VALUE
        ...
        if (screen_name != null) {
            ...
            builder.append("(SELECT ? IN (SELECT " + Filters.VALUE + " FROM "
                    + TABLE_FILTERED_USERS + "))");
        }
        if (source != null) {
            if (!selection_args.isEmpty()) {
            ...
            builder.append("(SELECT 1 IN (SELECT ? LIKE '%>'||" + TABLE_FILTERED_SOURCES + "."
                    + Filters.VALUE
    ...
    public static boolean isMyAccount(final Context context, final String screen_name) {
        ...
        final Cursor cur = resolver.query(Accounts.CONTENT_URI, new String[0], where, new String[] {
            screen_name
        },
                null);
        try {
    ...
    public static boolean isMyUserName(final Context context, final String screen_name) {
        ...
        for (final String account_screen_name : getAccountScreenNames(context)) {
            if (account_screen_name.equalsIgnoreCase(screen_name))
                return true;
        }
        return false;
    }
    ...
    public static boolean isNotificationsSilent(final Context context) {
        if (context == null)
            return false;
        final SharedPreferences prefs = context.getSharedPreferences(
                SILENT_NOTIFICATIONS_PREFERENCE_NAME,
                Context.MODE_PRIVATE);
    ...
    public static boolean isSameAccount(final Context context, final long account_id,
            final long user_id) {
        if (context == null || account_id <= 0 || user_id <= 0)
            return false;
    ...
    public static boolean isSameAccount(final Context context, final long account_id,
            final String screen_name) {
        if (context == null || account_id <= 0 || screen_name == null)
            return false;
    ...
    public static ContentValues makeAccountContentValues(final Configuration conf,
        ...
        values.put(Accounts.PROFILE_IMAGE_URL,
                ParseUtils.parseString(user.getProfileImageUrlHttps()));
        values.put(Accounts.PROFILE_BANNER_URL,
                ParseUtils.parseString(user.getProfileBannerImageUrl()));
    ...
    public static ContentValues makeCachedUserContentValues(final User user,
        ...
        values.put(CachedUsers.PROFILE_IMAGE_URL,
                large_profile_image ? getBiggerTwitterProfileImage(profile_image_url)
                        : profile_image_url);
        ...
        values.put(
                CachedUsers.URL_EXPANDED,
                url != null && urls != null && urls.length > 0 ? ParseUtils.parseString(urls[0]
                        .getExpandedURL())
                        : null);
    ...
    public static ContentValues makeDirectMessageContentValues(final DirectMessage message,
            final long account_id,
        ...
        if (sender == null || recipient == null)
            return null;
        final String sender_profile_image_url = ParseUtils.parseString(sender
                .getProfileImageUrlHttps());
        final String recipient_profile_image_url = ParseUtils.parseString(recipient
                .getProfileImageUrlHttps());
        ...
        switch (getTableId(uri)) {
            ...
            case TABLE_ID_DIRECT_MESSAGES_INBOX: {
                context.sendBroadcast(new Intent(
                        BROADCAST_RECEIVED_DIRECT_MESSAGES_DATABASE_UPDATED));
                break;
            }
            case TABLE_ID_DIRECT_MESSAGES_OUTBOX: {
    ...
    public static void openDirectMessagesConversation(final FragmentActivity activity,
            final long account_id,
        ...
        if (activity instanceof DualPaneActivity && ((DualPaneActivity) activity).isDualPaneMode()) {
            ...
            if (details_fragment instanceof DirectMessagesConversationFragment
                    && details_fragment.isAdded()) {
                ((DirectMessagesConversationFragment) details_fragment).showConversation(
            ...
            if (account_id > 0 && conversation_id > 0) {
                ...
                if (conversation_id > 0) {
                    builder.appendQueryParameter(QUERY_PARAM_CONVERSATION_ID,
                            String.valueOf(conversation_id));
                }
            }
    ...
    public static void openStatus(final Activity activity, final long account_id,
            final long status_id) {
        if (activity == null || account_id <= 0 || status_id <= 0)
            return;
        if (activity instanceof DualPaneActivity && ((DualPaneActivity) activity).isDualPaneMode()) {
            final DualPaneActivity dual_pane_activity = (DualPaneActivity) activity;
    ...
    public static void openStatuses(final Activity activity, final List<ParcelableStatus> statuses) {
        if (activity == null || statuses == null)
            return;
        final Bundle bundle = new Bundle();
        bundle.putParcelableArrayList(INTENT_KEY_STATUSES,
                new ArrayList<ParcelableStatus>(statuses));
        if (activity instanceof DualPaneActivity && ((DualPaneActivity) activity).isDualPaneMode()) {
            final DualPaneActivity dual_pane_activity = (DualPaneActivity) activity;
    ...
    public static void openStatusRetweeters(final Activity activity, final long account_id,
            final long status_id) {
        if (activity == null)
            return;
        if (activity instanceof DualPaneActivity && ((DualPaneActivity) activity).isDualPaneMode()) {
    ...
    public static void openTweetSearch(final Activity activity, final long account_id,
            final String query) {
        if (activity == null)
            return;
        if (activity instanceof DualPaneActivity && ((DualPaneActivity) activity).isDualPaneMode()) {
    ...
    public static void openUserFavorites(final Activity activity, final long account_id,
            final long user_id,
            final String screen_name) {
        if (activity == null)
            return;
    ...
    public static void openUserFollowers(final Activity activity, final long account_id,
            final long user_id,
            final String screen_name) {
        if (activity == null)
            return;
    ...
    public static void openUserFriends(final Activity activity, final long account_id,
            final long user_id,
            final String screen_name) {
        if (activity == null)
            return;
    ...
    public static void openUserListDetails(final Activity activity, final long account_id,
            final int list_id,
    ...
    public static void openUserListMembers(final Activity activity, final long account_id,
            final int list_id,
            final long user_id, final String screen_name, final String list_name) {
        if (activity == null)
            return;
    ...
    public static void openUserListMembers(final Activity activity, final ParcelableUserList list) {
        if (activity == null || list == null)
            return;
        openUserListMembers(activity, list.account_id, list.id, list.user_id,
                list.user_screen_name, list.name);
    }

    public static void openUserListMemberships(final Activity activity, final long account_id,
            final long user_id,
            final String screen_name) {
        if (activity == null || account_id <= 0 || user_id <= 0 && isEmpty(screen_name))
            return;
        if (activity instanceof DualPaneActivity && ((DualPaneActivity) activity).isDualPaneMode()) {
            final DualPaneActivity dual_pane_activity = (DualPaneActivity) activity;
    ...
    public static void openUserLists(final Activity activity, final long account_id,
            final long user_id,
            final String screen_name) {
        if (activity == null)
            return;
    ...
    public static void openUserListSubscribers(final Activity activity, final long account_id,
            final int list_id,
            final long user_id, final String screen_name, final String list_name) {
        if (activity == null)
            return;
    ...
    public static void openUserListSubscribers(final Activity activity,
            final ParcelableUserList list) {
        if (activity == null || list == null)
            return;
        openUserListSubscribers(activity, list.account_id, list.id, list.user_id,
                list.user_screen_name, list.name);
    }

    public static void openUserListTimeline(final Activity activity, final long account_id,
            final int list_id,
            final long user_id, final String screen_name, final String list_name) {
        if (activity == null)
            return;
    ...
    public static void openUserListTimeline(final Activity activity, final ParcelableUserList list) {
        if (activity == null || list == null)
            return;
        openUserListTimeline(activity, list.account_id, list.id, list.user_id,
                list.user_screen_name, list.name);
    }

    public static void openUserMentions(final Activity activity, final long account_id,
            final String screen_name) {
        if (activity == null)
            return;
        if (activity instanceof DualPaneActivity && ((DualPaneActivity) activity).isDualPaneMode()) {
    ...
    public static void openUserProfile(final Activity activity, final long account_id,
            final long user_id,
            final String screen_name) {
        if (activity == null || account_id <= 0 || user_id <= 0 && isEmpty(screen_name))
            return;
    ...
    public static void openUserTimeline(final Activity activity, final long account_id,
            final long user_id,
            final String screen_name) {
        if (activity == null)
            return;
    ...
    public static void setMenuForStatus(final Context context, final Menu menu,
        ...
        addIntentToMenu(context, more_submenu != null ? more_submenu.getSubMenu() : menu,
                extension_intent);
    }
    ...
    public static void setUserNickname(final Context context, final long user_id,
    ...
    public static void showErrorMessage(final Context context, final CharSequence message,
        ...
        if (context instanceof Activity) {
            final Crouton crouton = Crouton.makeText((Activity) context, message,
                    CroutonStyle.ALERT);
            final CroutonConfiguration.Builder cb = new CroutonConfiguration.Builder();
            cb.setDuration(long_message ? CroutonConfiguration.DURATION_LONG
                    : CroutonConfiguration.DURATION_SHORT);
            crouton.setConfiguration(cb.build());
            crouton.show();
        } else {
            final Toast toast = Toast.makeText(context, text, long_message ? Toast.LENGTH_LONG
                    : Toast.LENGTH_SHORT);
            toast.show();
        }
    }
    ...
    public static void showErrorMessage(final Context context, final CharSequence action,
        ...
        if (context instanceof Activity) {
            ...
            final Toast toast = Toast.makeText(context, message, long_message ? Toast.LENGTH_LONG
                    : Toast.LENGTH_SHORT);
            toast.show();
        }
    }
    ...
    public static void showInfoMessage(final Context context, final CharSequence message,
        ...
        if (context instanceof Activity) {
            final Crouton crouton = Crouton
                    .makeText((Activity) context, message, CroutonStyle.INFO);
            final CroutonConfiguration.Builder cb = new CroutonConfiguration.Builder();
            cb.setDuration(long_message ? CroutonConfiguration.DURATION_LONG
                    : CroutonConfiguration.DURATION_SHORT);
            crouton.setConfiguration(cb.build());
    ...
    public static void showInfoMessage(final Context context, final int resId,
            final boolean long_message) {
        if (context == null)
            return;
    ...
    public static void showOkMessage(final Context context, final CharSequence message,
            final boolean long_message) {
        if (context == null || isEmpty(message))
            return;
        if (context instanceof Activity) {
    ...
    public static void showOkMessage(final Context context, final int resId,
            final boolean long_message) {
        if (context == null)
            return;
            ...
            if (action != null) {
                if (te.exceededRateLimitation()) {
                    ...
                    final String next_reset_time = ParseUtils
                            .parseString(getRelativeTimeSpanString(System
                    ...
                    final String msg = TwitterErrorCodes
                            .getErrorMessage(context, te.getErrorCode());
    ...
    public static void showWarnMessage(final Context context, final CharSequence message,
        ...
        if (context instanceof Activity) {
            final Crouton crouton = Crouton
                    .makeText((Activity) context, message, CroutonStyle.WARN);
            final CroutonConfiguration.Builder cb = new CroutonConfiguration.Builder();
            cb.setDuration(long_message ? CroutonConfiguration.DURATION_LONG
                    : CroutonConfiguration.DURATION_SHORT);
            crouton.setConfiguration(cb.build());
    ...
    public static void showWarnMessage(final Context context, final int resId,
            final boolean long_message) {
        if (context == null)
            return;
    ...
    private static void parseEntities(final HtmlBuilder builder, final EntitySupport entities) {
        ...
        if (medias != null) {
            for (final MediaEntity media : medias) {
                ...
                if (media_url != null) {
                    builder.addLink(ParseUtils.parseString(media_url), media.getDisplayURL(),
                            media.getStart(),
                            media.getEnd());
                }
            }
