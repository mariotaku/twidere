...
public class HttpClientImpl implements twitter4j.http.HttpClient, HttpResponseCode {
            ...
            if (conf.getHttpProxyUser() != null && !conf.getHttpProxyUser().equals("")) {
                if (logger.isDebugEnabled()) {
                    ...
                    logger.debug("Proxy AuthPassword: "
                            + InternalStringUtil.maskString(conf.getHttpProxyPassword()));
                }
                client.getCredentialsProvider().setCredentials(
                        new AuthScope(conf.getHttpProxyHost(), conf.getHttpProxyPort()),
                        new UsernamePasswordCredentials(conf.getHttpProxyUser(), conf
    ...
    @Override
    public twitter4j.http.HttpResponse request(final twitter4j.http.HttpRequest req)
        ...
        try {
                        ...
                        for (final HttpParameter parameter : params) {
                            if (parameter.isFile()) {
                                ...
                                if (parameter.getFile() != null) {
                                    body = new FileBody(parameter.getFile(),
                                            parameter.getContentType());
                                } else {
                                    body = new InputStreamBody(parameter.getFileBody(),
            ...
            try {
                ...
                throw new TwitterException(
                        "Please check your APN settings, make sure not to use WAP APNs.", e);
    ...
    final static class TrustAllSSLSocketFactory extends SSLSocketFactory {
        ...
        TrustAllSSLSocketFactory(final KeyStore truststore) throws NoSuchAlgorithmException,
