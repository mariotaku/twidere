...
public class SlidingPaneView extends ViewGroup {
    ...
    public SlidingPaneView(final Context context, final AttributeSet attrs, final int defStyle) {
        ...
        mLeftSpacing = a.getDimensionPixelSize(R.styleable.SlidingPaneView_spacingLeft,
                spacingLeftDefault);
        final int spacingRightDefault = res
                .getDimensionPixelSize(R.dimen.default_slidepane_spacing_right);
        mRightSpacing = a.getDimensionPixelSize(R.styleable.SlidingPaneView_spacingRight,
                spacingRightDefault);
        ...
        final boolean shadowSlidable = a.getBoolean(R.styleable.SlidingPaneView_shadowSlidable,
                shadowSlidableDefault);

        mShadowWidth = a.getDimensionPixelSize(R.styleable.SlidingPaneView_shadowWidth, 0);
        final int shadowDrawableRes = a
                .getResourceId(R.styleable.SlidingPaneView_shadowDrawable, 0);
        ...
        mFlingDuration = a.getInteger(R.styleable.SlidingPaneView_flingDuration,
        ...
        if (rightPaneLayout == 0)
            throw new IllegalArgumentException();
        mRightPaneView = inflater.inflate(rightPaneLayout, mFadingRightPaneContainer, true);

        addView(mLeftPaneLayout, new LayoutParams(LayoutParams.MATCH_PARENT,
        ...
        mRightPaneLayout.addView(mRightPaneContainer, LayoutParams.MATCH_PARENT,
                LayoutParams.MATCH_PARENT);

        mRightPaneContainer.addView(mRightPaneBackgroundView, LayoutParams.MATCH_PARENT,
                LayoutParams.MATCH_PARENT);
        mRightPaneContainer.addView(mFadingRightPaneContainer, LayoutParams.MATCH_PARENT,
                LayoutParams.MATCH_PARENT);
    ...
    public void setFadeType(final int type) {
        if (type != FADE_NONE && type != FADE_LEFT && type != FADE_RIGHT && type != FADE_BOTH)
            return;
    ...
    protected void onLayout(final boolean changed, final int l, final int t, final int r,
        ...
        for (int i = 0; i < childrenCount; ++i) {
            ...
            if (v == mRightPaneLayout) {
                ...
                v.layout(l + mLeftSpacing - shadowWidth, t,
                        l + mLeftSpacing + v.getMeasuredWidth(),
    ...
    protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
        ...
        for (int i = 0; i < childrenCount; ++i) {
            ...
            if (v == mLeftPaneLayout) {
                ...
                final int shadowWidth = isShadowVisible() ? mShadowWidth : 0;
                final int contentWidth = MeasureSpec.getSize(widthMeasureSpec) - mLeftSpacing
                        + shadowWidth;
        ...
        for (int i = count - 1; i >= 0; i--) {
            ...
            if (child.isShown() || child.getAnimation() != null) {
                ...
                if (frame.contains(scrolledXInt, scrolledYInt)) {
                    ...
                    if (isTouchEventHandled(child, ev))
                        return true;
                }
            }
        }
    ...
    private class ContentScrollController implements Runnable {
        ...
        private void hideRightPane(final int duration) {
            ...
            if (mRightPaneLayout.getMeasuredWidth() == 0
                    || mRightPaneLayout.getMeasuredHeight() == 0)
                return;
    ...
    private static class ScrollTouchInterceptor implements TouchInterceptor {
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE: {
                    ...
                    if (Math.abs(mTempDeltaX) > Math.abs(deltaY) && !mIsVerticalScrolling
                            && Math.abs(mTotalMoveX) >= mScaledTouchSlop)
                        return true;
        ...
        public boolean onTouchEvent(final ViewGroup view, final MotionEvent event) {
            if (mIsVerticalScrolling && !mController.isScrolling() || mShouldDisableScroll)
                return true;
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN: {
                    mTempDeltaX = 0;
