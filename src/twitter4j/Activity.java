...
public interface Activity extends TwitterResponse, Comparable<Activity>, Serializable {
    ...
    public static enum Action implements Serializable {
        FAVORITE(0x1), FOLLOW(0x2), MENTION(0x3), REPLY(0x4), RETWEET(0x5), LIST_MEMBER_ADDED(0x06), LIST_CREATED(
                0x07);
        ...
        public static Action fromString(final String string) {
            ...
            if ("reply".equalsIgnoreCase(string))
                return REPLY;
            if ("retweet".equalsIgnoreCase(string))
                return RETWEET;
            if ("list_member_added".equalsIgnoreCase(string))
                return LIST_MEMBER_ADDED;
            if ("list_created".equalsIgnoreCase(string))
                return LIST_CREATED;
