...
abstract class TwitterBaseImpl implements OAuthSupport, HttpResponseListener {
    ...
    private List<RateLimitStatusListener> rateLimitStatusListeners = new ArrayList<RateLimitStatusListener>(
            0);
    ...
    public synchronized AccessToken getOAuthAccessToken() throws TwitterException {
        ...
        if (auth instanceof BasicAuthorization) {
            ...
            if (auth instanceof OAuthAuthorization) {
                ...
                oauthAccessToken = oauthAuth.getOAuthAccessToken(basicAuth.getUserId(),
            ...
            if (auth instanceof XAuthAuthorization) {
                ...
                oauthAccessToken = oauthAuth.getOAuthAccessToken(xauth.getUserId(),
                        xauth.getPassword());
            } else {
     ...
     */
    @Override
    public synchronized AccessToken getOAuthAccessToken(final RequestToken requestToken)
     ...
     */
    @Override
    public synchronized AccessToken getOAuthAccessToken(final String oauthVerifier)
            throws TwitterException {
     ...
     */
    @Override
    public synchronized AccessToken getOAuthAccessToken(final String screenName,
    ...
    public void httpResponseReceived(final HttpResponseEvent event) {
        if (rateLimitStatusListeners.size() != 0) {
            ...
            if (te != null) {
                ...
                rateLimitStatus = InternalJSONFactoryImpl
                        .createRateLimitStatusFromResponseHeader(res);
                statusCode = res.getStatusCode();
            }
            if (rateLimitStatus != null) {
                final RateLimitStatusEvent statusEvent = new RateLimitStatusEvent(this,
                        rateLimitStatus,
    ...
    protected void addParameterToList(final List<HttpParameter> params, final String paramName,
            final Boolean param) {
        if (param != null) {
    ...
    protected HttpResponse get(final String url, final String sign_url,
            final HttpParameter... parameters)
            throws TwitterException {
    ...
    protected HttpParameter[] mergeParameters(final HttpParameter[] params1,
            final HttpParameter... params2) {
    ...
    protected HttpResponse post(final String url, final String sign_url,
            final HttpParameter... parameters)
            throws TwitterException {
        // intercept HTTP call for monitoring purposes
        HttpResponse response = null;
