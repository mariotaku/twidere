...
public final class HttpParameter implements Comparable<HttpParameter> {
    ...
    public String toString() {
        return "PostParameter{" + "name='" + name + '\'' + ", value='" + value + '\'' + ", file="
                + file
                + ", fileBody=" + fileBody + '}';
    }
    ...
    public static String encodeParameters(final HttpParameter[] httpParams) {
        ...
        for (int j = 0; j < httpParams.length; j++) {
            if (httpParams[j].isFile())
                throw new IllegalArgumentException("parameter [" + httpParams[j].name
                        + "]should be text");
            if (j != 0) {
                buf.append("&");
            }
    ...
    public static HttpParameter[] getParameterArray(final String name1, final String value1,
            ...
            final String value2) {
        return new HttpParameter[] {
                new HttpParameter(name1, value1), new HttpParameter(name2, value2)
        };
    }
