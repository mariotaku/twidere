...
class ActivityJSONImpl extends TwitterResponseImpl implements Activity {
    ...
    public int compareTo(final Activity another) {
        ...
        if (this_date == null || that_date == null)
            return 0;
    ...
    public String toString() {
        return "ActivityJSONImpl{action=" + action + ", createdAt=" + createdAt + ", sources="
                + sources
                + ", targetUsers=" + targetUsers + ", targetObjects=" + targetObjectStatuses
                + ", targetStatuses="
                + targetStatuses + ", maxPosition=" + maxPosition + ", minPosition=" + minPosition
                + ", targetObjectsSize=" + targetObjectsSize + ", targetsSize=" + targetsSize
                + ", sourcesSize="
                + sourcesSize + "}";
    }
    ...
    final void init(final JSONObject json) throws TwitterException {
        try {
            ...
            } else if (action == Action.FOLLOW || action == Action.MENTION
                    || action == Action.LIST_MEMBER_ADDED) {
            ...
            if (action == Action.LIST_MEMBER_ADDED) {
                ...
                for (int i = 0; i < target_objects_size; i++) {
                    targetObjectUserLists[i] = new UserListJSONImpl(
                            target_objects_array.getJSONObject(i));
                }
                ...
                for (int i = 0; i < target_objects_size; i++) {
                    targetObjectStatuses[i] = new StatusJSONImpl(
                            target_objects_array.getJSONObject(i));
                }
            }
    ...
    /* package */
    static ResponseList<Activity> createActivityList(final HttpResponse res,
