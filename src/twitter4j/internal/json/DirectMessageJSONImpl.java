...
/* package */final class DirectMessageJSONImpl extends TwitterResponseImpl implements DirectMessage {
    ...
    /* package */DirectMessageJSONImpl(final HttpResponse res, final Configuration conf)
    ...
    public String toString() {
        return "DirectMessageJSONImpl{id=" + id + ", text=" + text + ", rawText=" + rawText
                + ", senderId=" + senderId
                + ", recipientId=" + recipientId + ", createdAt=" + createdAt
                + ", senderScreenName="
                + senderScreenName + ", recipientScreenName=" + recipientScreenName
                + ", userMentionEntities="
                + userMentionEntities + ", urlEntities=" + urlEntities + ", hashtagEntities="
                + hashtagEntities
        ...
        if (!json.isNull("entities")) {
            try {
                ...
                if (!entities.isNull("user_mentions")) {
                    ...
                    for (int i = 0; i < len; i++) {
                        userMentionEntities[i] = new UserMentionEntityJSONImpl(
                ...
                if (!entities.isNull("hashtags")) {
                    ...
                    for (int i = 0; i < len; i++) {
                        hashtagEntities[i] = new HashtagEntityJSONImpl(
    ...
    /* package */
    static ResponseList<DirectMessage> createDirectMessageList(final HttpResponse res,
            final Configuration conf)
            throws TwitterException {
        try {
            final JSONArray list = res.asJSONArray();
            final int size = list.length();
            final ResponseList<DirectMessage> directMessages = new ResponseListImpl<DirectMessage>(
                    size, res);
