...
class FriendshipJSONImpl implements Friendship {
    ...
    public String toString() {
        return "FriendshipJSONImpl{" + "id=" + id + ", name='" + name + '\'' + ", screenName='"
                + screenName + '\''
                + ", following=" + following + ", followedBy=" + followedBy + '}';
    }

    /* package */
    static ResponseList<Friendship> createFriendshipList(final HttpResponse res,
            final Configuration conf)
            throws TwitterException {
        try {
            final JSONArray list = res.asJSONArray();
            final int size = list.length();
            final ResponseList<Friendship> friendshipList = new ResponseListImpl<Friendship>(size,
                    res);
